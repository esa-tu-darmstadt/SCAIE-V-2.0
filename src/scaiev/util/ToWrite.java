...
public class ToWrite {
  ...
  public boolean prereq_val = true; // if prereq with previous condition, the start value should be false. If prereq with no previous
                                    // condition = run once, start value should be true. If no prereq, set to whatever
   ...
   * @param prereq Set to true to enable scanning for a 'prerequisite' line that should appear before the match. The ToWrite object can
   *     match once per prerequisite line.
   * @param prereq_val Initial value of prereq_val, which marks whether this ToWrite descriptor should be considered for matching before the
   *     prerequisite line is found.
   *                   During file processing, will be set to true when the prerequisite line is found, and reset to false after a match
   * with this ToWrite object is found. In case prereq==false, this needs to be set to true, as otherwise, no matches can be produced with
   * this object.
   ...
   * @param in_module The name of the module to search within (e.g. in the block starting after the line {@code 'module <name>'} and ending
   *     with {@code 'endmodule'} in Verilog), or "" to search in the whole file.
   */
  ...
  public String toString() {
    return "Text to be added: " + text + ". Prereq requested: " + prereq + ". Text to be added before greped line: " + before +
        ". In module " + in_module;
  }
}