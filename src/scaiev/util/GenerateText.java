...
public abstract class GenerateText {
  ...
  public GenerateText(BNode user_BNode) { this.BNode = user_BNode; }
  ...
  public String GetDict(DictWords input) { return dictionary.get(input); }

  public String GetDictModule() { return dictionary.get(DictWords.module); }

  public String GetDictEndModule() { return dictionary.get(DictWords.endmodule); }
    ...
    for (int i = 0; i <= encodingRaw.length(); ++i) // encodingRaw.length() is in range (intentionally).
    {
      if (i == encodingRaw.length() || encodingRaw.charAt(i) == '-') {
        if (next_ibitmax < i) {
          ...
          body += String.format("(%s%s%d%s%s %s %s)", rdInstr, dictionary.get(DictWords.bitsselectLeft), cur_bitmax,
                                (cur_bitmin < cur_bitmax) ? String.format(" %s %d", dictionary.get(DictWords.bitsRange), cur_bitmin) : "",
                                dictionary.get(DictWords.bitsselectRight), dictionary.get(DictWords.ifeq),
  ...
  public String CreateValidEncodingOneInstr(String ISAX, HashMap<String, SCAIEVInstr> allISAXes, String rdInstr) {
    ...
    if (!allISAXes.get(ISAX).GetEncodingOp(Lang.None).equals("-------"))
      body += CreateValidSubencodings(allISAXes.get(ISAX).GetEncodingOp(Lang.None), rdInstr, 0,
                                      6); // "("+rdInstr+dictionary.get(DictWords.bitsselectLeft)+"6 "+dictionary.get(DictWords.bitsRange)+"
                                          // 0"+dictionary.get(DictWords.bitsselectRight)+" "+dictionary.get(DictWords.ifeq)+"
                                          // "+allISAXes.get(ISAX).GetEncodingOp(getLang())+")";
    if (!allISAXes.get(ISAX).GetEncodingF3(Lang.None).equals("---"))
      body += String.format("%s%s", body.isEmpty() ? "" : (" " + dictionary.get(DictWords.logical_and) + " "),
                            CreateValidSubencodings(allISAXes.get(ISAX).GetEncodingF3(Lang.None), rdInstr, 12, 14));
    if (!allISAXes.get(ISAX).GetEncodingF7(Lang.None).equals("-------"))
      body += String.format("%s%s", body.isEmpty() ? "" : (" " + dictionary.get(DictWords.logical_and) + " "),
                            CreateValidSubencodings(allISAXes.get(ISAX).GetEncodingF7(Lang.None), rdInstr, 25, 31));
    body = String.format("(%s)", body);
    return body;
  }

  public String CreateAddrEncoding(HashSet<String> lookAtISAX, HashMap<String, SCAIEVInstr> allISAXes, String rdInstr,
                                   SCAIEVNode operation) {
    String body = "";
  ...
  //	public String  CreateValidEncoding(HashSet<String> lookAtISAX, HashMap <String,SCAIEVInstr>  allISAXes, String rdInstr, SCAIEVNode
  // operation) { 		String body = ""; 		for (String ISAX  :  lookAtISAX) { 			if(!body.isEmpty())
  // body += " "+dictionary.get(DictWords.logical_or)+"
  //"; 			body += CreateValidEncodingOneInstr(ISAX, allISAXes, rdInstr);
  //
  //			Iterator<Scheduled> snode_iter = allISAXes.get(ISAX).GetSchedWithIterator(operation, _snode ->
  //_snode.HasAdjSig(AdjacentNode.validReq)); 			if (snode_iter.hasNext()) { 				body += "
  //"+dictionary.get(DictWords.logical_and)+" ("; 				boolean is_first = true; 				do {
  // Scheduled snode = snode_iter.next(); 					if (!is_first) 						body
  // += " "+dictionary.get(DictWords.logical_or)+" "; 					body +=
  // this.CreateLocalNodeName(BNode.GetAdjSCAIEVNode(operation, AdjacentNode.validReq), snode.GetStartCycle(), "");
  // is_first = false; 				} while (snode_iter.hasNext()); 				body += ")";
  //			}
  //			body += ")";
  //		}
  //		return body;
  ...
  public void GenerateAllInterfaces(String topModule, HashMap<SCAIEVNode, HashMap<PipelineStage, HashSet<String>>> op_stage_instr,
                                    HashMap<String, SCAIEVInstr> ISAXes, Core core, SCAIEVNode specialCase) {
    logger.info("Generating interface for: " +
                op_stage_instr.entrySet()
                    .stream()
                    .map(entryOpStIn -> {
                      return Map.entry(entryOpStIn.getKey(),
                                       entryOpStIn.getValue()
                                           .entrySet()
                                           .stream()
                                           .map(entryStIn
                                                -> Map.entry(entryStIn.getKey().getName(),
                                                             entryStIn.getValue().stream().map(instr -> "\"" + instr + "\"").toList()))
                                           .toList());
                    })
                    .toList());
    ...
    for (SCAIEVNode operation : op_stage_instr.keySet()) {
      for (PipelineStage stage : op_stage_instr.get(operation).keySet()) {
        // boolean spawnValid = !operation.nameParentNode.isEmpty() && BNode.HasSCAIEVNode(operation.nameParentNode) && operation.isSpawn()
        ...
        if (!operation.equals(specialCase)) {
          ...
          //							if (!adjOperation.nameCousinNode.isEmpty() &&
          // BNode.HasSCAIEVNode(adjOperation.nameCousinNode)
          //								&& !addedFamilyNodes.add(CreateNodeName(adjOperation, stage, ""))) {
          //								//Prevent doubled interfaces nodes that will go by their family
          // name.
          //								//Example: RdMem_spawn_validReq and WrMem_spawn_validReq would each
          // be added as Mem_spawn_validReq 								continue;
          //							}
