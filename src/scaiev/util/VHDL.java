...
public class VHDL extends GenerateText {
  ...
  public void UpdateInterface(String top_module, SCAIEVNode operation, String instr, PipelineStage stage, boolean top_interface,
    ...
    while (!prev_module.contentEquals(top_module)) {
      ...
      if (!current_module.contentEquals(top_module) ||
          top_interface) { // top file should just instantiate signal in module instantiation and not generate top interface
        logger.debug("INTEGRATE. DEBUG. Inserting in components.vhd with prereq " + current_module);
        this.toFile.UpdateContent(
            coreBackend.ModInterfFile(current_module), "port (",
            new ToWrite(CreateTextInterface(operation, stage, instr, special_case), true, false, " " + current_module + " "));
        this.toFile.UpdateContent(
            coreBackend.ModFile(current_module), "port (",
            new ToWrite(CreateTextInterface(operation, stage, instr, special_case), true, false, " " + current_module + " "));
      } else if (current_module.contentEquals(top_module)) {
        this.toFile.UpdateContent(coreBackend.ModFile(current_module), "architecture rtl",
                                  new ToWrite(CreateDeclSig(operation, stage, instr), true, false, " " + current_module + " "));
      }
      ...
      if (prev_module.contentEquals("")) {
        ...
        /*
                ...
                this.toFile.UpdateContent(coreBackend.ModFile(current_module),"architecture rtl", new
        ToWrite(assign_lineToBeInserted,false,true,"",false));
        }*/
      } else {
        ...
        this.toFile.UpdateContent(coreBackend.ModFile(current_module), "port map (",
                                  new ToWrite(instance_sig, true, false, ": " + prev_module));
      }
  ...
  public String CreateTextInterface(SCAIEVNode operation, PipelineStage stage, String instr, boolean special_case) {
    ...
    if (special_case)
      interf_lineToBeInserted = sig_name + " : "
                                + "buffer"
                                + " " + size + ";-- ISAX\n";
