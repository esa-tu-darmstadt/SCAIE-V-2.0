...
public class DRC {
  ...
  public DRC(HashMap<String, SCAIEVInstr> ISAXes, HashMap<SCAIEVNode, HashMap<PipelineStage, HashSet<String>>> op_stage_instr, Core core,
             BNode BNodes) {
    this.ISAXes = ISAXes;
  ...
  public boolean HasFatalError() { return hasFatalError; }
  ...
  public void CheckSchedErr() {
      ...
      for (var stage_instr_entry : op_stage_instr_entry.getValue().entrySet()) {
        ...
        if (!operation.isSpawn() && !(stage.getKind() == StageKind.Sub && operation.equals(BNodes.RdAnyValid))) {
          ...
          if ((!operation.isInput && !start_constrain_cycle.isAroundOrBefore(stage, false)) ||
            ...
            String message =
                "For instruction(s) " + stage_instr_entry.getValue() + ", node " + operation + " was scheduled in the wrong cycle: ";
    ...
    if (op_stage_instr.containsKey(BNodes.WrMem_spawn) && op_stage_instr.containsKey(BNodes.RdMem_spawn)) {
      if (op_stage_instr.get(BNodes.WrMem_spawn)
              .values()
              .stream()
              .flatMap(instrCollection -> instrCollection.stream())
              .anyMatch(instrWr
                        -> op_stage_instr.get(BNodes.RdMem_spawn).values().stream().anyMatch(instrRdSet -> instrRdSet.contains(instrWr)))) {
    ...
    //					return (instrSet==null ? Stream.<String>empty() : instrSet.stream()).map(instr -> Map.entry(node,
    // ISAXes.get(instr)));
    //				}));
    Supplier<Stream<Map.Entry<SCAIEVNode, SCAIEVInstr>>> makeSpawnInstructionsStream = ()
        -> this.op_stage_instr.entrySet()
               ...
               .flatMap(
                   op_stage_instr_entry
                   -> op_stage_instr_entry.getValue().entrySet().stream().map(
                       stage_instr_entry
                       -> Map.entry(op_stage_instr_entry.getKey(), stage_instr_entry.getValue()))) /* all operation-Set<ISAX name> pairs */
               .flatMap(op_instr_entry
                        -> op_instr_entry.getValue()
                               .stream()
                               .filter(instrName -> !instrName.isEmpty())
    ...
    List<String> instructionsMissingDynamicValidReq =
        makeSpawnInstructionsStream.get()
            .filter(node_instr_pair -> {
              return !node_instr_pair.getValue().HasNoOp() && node_instr_pair.getValue().GetRunsAsDynamic() &&
    ...
    if (!instructionsMissingDynamicValidReq.isEmpty()) {
      logger.fatal(
          "DRC - Dynamic memory operations must come with a validReq signal. Add the `has valid: true` option to the ISAX description yaml."
              + " Violating instructions: {}",
          instructionsMissingDynamicValidReq);
      hasFatalError = true;
    }
    List<String> instructionsViolatingMissingAddrSize =
        makeSpawnInstructionsStream.get()
            .filter(
                node_instr_pair
                -> !node_instr_pair.getValue().HasNoOp() && node_instr_pair.getValue().GetRunsAsDynamic() &&
    ...
    if (!instructionsViolatingMissingAddrSize.isEmpty()) {
      logger.warn(
          "DRC - Dynamic memory operations should come with addr and size signals if they can run several or zero times over the "
          ...
          instructionsViolatingMissingAddrSize);
    }
    List<String> opinstrViolatingSemicoupledSingleCommittable =
        makeSpawnInstructionsStream.get()
            .filter(node_instr_pair
                    -> !node_instr_pair.getValue().GetRunsAsDecoupled() &&
            ...
            .filter(node_instr_pair -> {
              ...
              //'iter.count() > 0'
              if (!iter.hasNext()) {
                logger.error("DRC - Cannot find a scheduled node matching ({}:{})", node_instr_pair.getKey().name,
                             node_instr_pair.getValue().GetName());
                return false;
              }
              //'iter.count() > 1'
              iter.next();
    ...
    if (!opinstrViolatingSemicoupledSingleCommittable.isEmpty()) {
      logger.fatal("DRC - For semi-coupled spawn, only a single instance of each committable operation type is allowed per ISAX."
                       + " Violating operation-instruction pairs: {}",
                   opinstrViolatingSemicoupledSingleCommittable);
      hasFatalError = true;
    }
        ...
        this.op_stage_instr.entrySet()
            ...
            .filter(op_stage_instr_entry
                    -> !op_stage_instr_entry.getKey().isSpawn() &&
                           BNodes.GetMySpawnNode(op_stage_instr_entry.getKey())
                               .isPresent()) /* all non-spawn nodes in op_stage_instr that have a spawn variant */
            .flatMap(op_stage_instr_entry
                     -> op_stage_instr_entry.getValue()
                            .entrySet()
                            .stream()
                            .filter(stage_instr_entry
                                    -> stage_instr_entry.getKey().getKind() == StageKind.Sub) /* all such operations in a sub-pipeline */
                            .map(stage_instr_entry
                                 -> Map.entry(op_stage_instr_entry.getKey(),
                                              stage_instr_entry.getValue()))) /* all operation-Set<ISAX name> pairs */
            .flatMap(op_instr_entry
                     -> op_instr_entry.getValue().stream().map(
                         instrName -> Map.entry(op_instr_entry.getKey(), instrName))) /* all operation-<ISAX name> pairs */
            .distinct()
    ...
    if (!opinstrViolatingSemicoupledNonspawnShouldBeSpawn.isEmpty()) {
      logger.fatal(
          "DRC - Encountered currently not supported constellation: Non-spawn operation (that has a spawn counterpart) in a spawn pipeline."
              + " This may be the case for semi-coupled ISAXes if the operation is used within the constrained range but not before the "
              + "'semi-coupled' execute stage chosen by SCAIE-V."
              + " Violating operation-instruction pairs: {}",
          opinstrViolatingSemicoupledNonspawnShouldBeSpawn);
      hasFatalError = true;
    }

    List<String> instructionsViolatingDynamicNeedsRdAnyValid =
        makeSpawnInstructionsStream.get()
            .map(op_instr -> op_instr.getValue())
            .distinct() /* all ISAXes with spawn ops */
            .filter(instr
                    -> instr.GetRunsAsDynamic() /* select dynamic spawn ISAXes... */
                           && !op_stage_instr   /* .. that do not have RdAnyValid in their dynamic spawn stage */
                                   .getOrDefault(BNodes.RdAnyValid, new HashMap<>())
                                   .entrySet()
                                   .stream()
                                   .anyMatch(stage_instrs
                                             -> stage_instrs.getValue().contains(instr.GetName()) &&
                                                    stage_instrs.getKey().getKind() ==
                                                        StageKind.Sub /* last sub stage of a dynamic ISAX -> dynamic stage */
    ...
    if (!instructionsViolatingDynamicNeedsRdAnyValid.isEmpty()) {
      logger.fatal("DRC - Encountered a dynamic spawn instruction that does not provide RdAnyValid in its spawn stage."
                       + " Violating instructions: {}",
                   instructionsViolatingDynamicNeedsRdAnyValid);
      hasFatalError = true;
    }

    List<String> opinstrViolatingNoWrFlushInSubpipeline =
        this.op_stage_instr.getOrDefault(BNodes.WrFlush, new HashMap<>())
            .entrySet()
            .stream()
    ...
    if (!opinstrViolatingNoWrFlushInSubpipeline.isEmpty()) {
      logger.fatal("DRC - A spawn instruction invokes WrFlush in its sub-pipeline, which is not allowed."
                       + " Violating instructions: {}",
                   opinstrViolatingNoWrFlushInSubpipeline);
      hasFatalError = true;
    }

    List<String> opinstrViolatingNoWrStallInSubpipeline =
        this.op_stage_instr.getOrDefault(BNodes.WrStall, new HashMap<>())
            .entrySet()
            .stream()
            .filter(stage_instr_entry
                    -> stage_instr_entry.getKey().getKind() == StageKind.Sub && stage_instr_entry.getKey().getPrev().size() > 0)
            .flatMap(stage_instr_entry -> stage_instr_entry.getValue().stream()) /* all ISAX names */
            .distinct()
            .toList();
    if (!opinstrViolatingNoWrStallInSubpipeline.isEmpty()) {
      logger.fatal("DRC - A spawn instruction uses WrStall within its sub-pipeline beyond the entry stage. Stall propagation within "
                       + "sub-pipelines is currently not implemented."
                       + " Violating instructions: {}",
                   opinstrViolatingNoWrStallInSubpipeline);
      hasFatalError = true;
    }
  }
  ...
  public void CheckEncPresent() {
    for (SCAIEVInstr instr : ISAXes.values()) {
      ...
      if (instr.GetEncodingF3(Lang.None).isEmpty() || instr.GetEncodingF7(Lang.None).isEmpty() ||
          instr.GetEncodingOp(Lang.None).isEmpty()) {
