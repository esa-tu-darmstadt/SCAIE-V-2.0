...
public class FNode {
  ...
  public SCAIEVNode WrRD = new SCAIEVNode("WrRD", datawidth, true) {
    {
      DH = true;
      validBy = AdjacentNode.validData;
    }
  };
  public SCAIEVNode WrMem = new SCAIEVNode("WrMem", datawidth, true) {
    {
      elements = 2;
      validBy = AdjacentNode.validData;
    }
  }; // 2 is a dummy value for the system to know that this interef requires mandatory addr signal to core. {{this.familyName = "Mem";}}; //
     // input data to core
  public SCAIEVNode RdMem = new SCAIEVNode("RdMem", datawidth, false) {
    {
      elements = 2;
      validBy = AdjacentNode.validResp;
      tags.add(NodeTypeTag.nonStaticReadResult);
    }
  }; // 2 is a dummy value for the system to know that this interef requires mandatory addr signal to core. {{this.familyName = "Mem";
     // nameCousinNode = WrMem.name; WrMem.nameCousinNode = this.name; }}; // output data from core
  public SCAIEVNode WrPC = new SCAIEVNode("WrPC", datawidth, true) {
  ...
  public SCAIEVNode RdRS1 = new SCAIEVNode("RdRS1", datawidth, false) {
    { tags.add(NodeTypeTag.staticReadResult); }
  };
  public SCAIEVNode RdRS2 = new SCAIEVNode("RdRS2", datawidth, false) {
    { tags.add(NodeTypeTag.staticReadResult); }
  };
  public SCAIEVNode RdRD = new SCAIEVNode("RdRD", datawidth, false) {
    { tags.add(NodeTypeTag.staticReadResult); }
  };
  public SCAIEVNode RdInstr = new SCAIEVNode("RdInstr", 32, false) {
    { tags.add(NodeTypeTag.staticReadResult); }
  };
  public SCAIEVNode RdIValid = new SCAIEVNode("RdIValid", 1, false) {
    {
      oneInterfToISAX = false;
      tags.add(NodeTypeTag.staticReadResult);
    }
  }; //{{RdIValid.forEachInstr = true;}}; // interface to be generated for each Instr
  ...
  /**
   * Interface for dynamic decoupled ISAXes: Apply WrStall to the stage of the RdFence input until no more instructions of this ISAX are
   * in-flight
   */
  ...
  public SCAIEVNode RdStall = new SCAIEVNode("RdStall", 1, false) {
    {
      oneInterfToISAX = false;
      tags.add(NodeTypeTag.perStageStatus);
    }
  };
  public SCAIEVNode WrStall = new SCAIEVNode("WrStall", 1, true) {
    {
      oneInterfToISAX = false;
      tags.add(NodeTypeTag.perStageStatus);
    }
  };
  public SCAIEVNode RdFlush = new SCAIEVNode("RdFlush", 1, false) {
    {
      oneInterfToISAX = false;
      tags.add(NodeTypeTag.perStageStatus);
    }
  };
  public SCAIEVNode WrFlush = new SCAIEVNode("WrFlush", 1, true) {
    {
      oneInterfToISAX = false;
      tags.add(NodeTypeTag.perStageStatus);
    }
  ...
  public boolean IsUserFNode(SCAIEVNode node) { return user_FNode.contains(node); }
  ...
  public boolean HasSCAIEVNode(String nodeName) { return HasSCAIEVFNode(nodeName); }
