...
public class SCAIEVNode {
  ...
  public enum NodeTypeTag {
    /**
       The node is a read result that remains valid for the lifetime of an instruction. Should not be combined with {@link
       NodeTypeTag#nonUniqueReadResult}.
     */
    staticReadResult,
    /**
  ...
  /**
   * By default true for all spawn nodes. WrPC_spawn should have it false, as no multiple spawn instructions are allowed to concurrently
   * update PC..it doesn't make sense
   */
  ...
  public boolean DefaultMandatoryAdjSig() {
    if (this.adjSignal.equals(AdjacentNode.validReq) || (this.isSpawn() && this.adjSignal.equals(AdjacentNode.validResp)) ||
        (this.adjSignal.equals(AdjacentNode.addrReq) && (elements > 1)))
      return true;
    else
      return false;
  }
