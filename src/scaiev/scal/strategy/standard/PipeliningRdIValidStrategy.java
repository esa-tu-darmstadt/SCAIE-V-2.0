...
/**
 * Implements RdIValid with pipelining support, also taking combinational 'RdFlush' and core-specific conditions of the destination stage
 * into account.
 */
public class PipeliningRdIValidStrategy extends MultiNodeStrategy {
   ...
   */
  public PipeliningRdIValidStrategy(StrategyBuilders strategyBuilders, Verilog language, BNode bNodes, Core core,
                                    PipelineFront minPipelineFront, HashMap<String, SCAIEVInstr> allISAXes,
                                    Function<PipelineStage, RdIValidStageDesc> stage_getRdIValidDesc) {
    ...
    this.ivalidStrategy = strategyBuilders.buildNodeRegPipelineStrategy(
        language, bNodes, minPipelineFront, true, true, true,
        key
        -> key.getNode().equals(bNodes.RdIValid) &&
  ...
  protected class RdIValidPostPipeliningBuilder extends RdIValidBuilder {
    ...
    protected static final Purpose marker_ReevalPipeliningRdIValid =
        new Purpose("marker_ReevalPipeliningRdIValid", true, Optional.empty(), List.of());
  }
  ...
  public void implement(Consumer<NodeLogicBuilder> out, Iterable<NodeInstanceDesc.Key> nodeKeys, boolean isLast) {
    ...
    while (nodeKeyIter.hasNext()) {
      ...
      if (!((nodeKey.getPurpose().matches(Purpose.REGULAR) || nodeKey.getPurpose().matches(Purpose.WIREDIN) ||
             nodeKey.getPurpose().matches(Purpose.PIPEDIN)) &&
      ...
      NodeInstanceDesc.Key additionalKey =
      ...
      if (ivalidBuilders.size() > 0) {
        ...
        NodeLogicBuilder mergedBuilder = NodeLogicBuilder.fromFunction(builderName, (registry, aux) -> {
          ...
          Optional<NodeInstanceDesc> pipedinNode_opt =
              logicBlock.outputs.stream().filter(output -> output.getKey().equals(pipedinKey)).findFirst();
      ...
      if (nodeKey.getPurpose().matches(Purpose.REGULAR) || nodeKey.getPurpose().matches(Purpose.WIREDIN)) {
        ...
        if (additionalIValidBuilder != null) {
          ...
          keysToImplement.put(additionalKey, new RdIValidPostPipeliningBuilder(makeBuilderName.get() + "_WIREDIN+REGULAR", nodeKey));
