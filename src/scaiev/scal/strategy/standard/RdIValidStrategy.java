...
public class RdIValidStrategy extends MultiNodeStrategy {
   ...
   */
  public RdIValidStrategy(Verilog language, BNode bNodes, Core core, HashMap<String, SCAIEVInstr> allISAXes,
  ...
  public void implement(Consumer<NodeLogicBuilder> out, Iterable<NodeInstanceDesc.Key> nodeKeys, boolean isLast) {
    ...
    while (nodeKeyIter.hasNext()) {
      ...
      NodeInstanceDesc.Key genericKey =
          new NodeInstanceDesc.Key(nodeKey.getNode(), nodeKey.getStage(), nodeKey.getISAX()); // Store without the specific Purpose
      RdIValidBuilder existingBuilder = existingBuilders.get(genericKey);
      if (existingBuilder == null) {
        var newBuilder = new RdIValidBuilder("RdIValidStrategy (" + nodeKey.toString() + ")", nodeKey, language, bNodes, core, allISAXes,
                                             stage_getRdIValidDesc);
