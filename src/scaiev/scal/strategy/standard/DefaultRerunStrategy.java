  ...
  public Optional<NodeLogicBuilder> implement(NodeInstanceDesc.Key nodeKey) {
    if (nodeKey.getPurpose().matches(Purpose.MARKER_INTERNALIMPL_PIN) && nodeKey.getNode().equals(bNodes.WrRerunNext)) {
      ...
      if (!useDefaultRerunNextImplementation(nodeKey.getStage())) {
        return Optional.of(
            NodeLogicBuilder.fromFunction("DefaultRerunStrategy_RequestCoreSpecific_" + nodeKey.getStage().getName(), registry -> {
      ...
      return Optional.of(NodeLogicBuilder.fromFunction("DefaultRerunStrategy_" + nodeKey.getStage().getName(), (registry, aux) -> {
        ...
        ret.outputs.add(new NodeInstanceDesc(new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrPC, nodeKey.getStage(), "", aux), rdPCExpr,
        ...
        ret.outputs.add(new NodeInstanceDesc(new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrPC_valid, nodeKey.getStage(), "", aux),
                                             rerunWrPCValidWireName, ExpressionType.WireName));
