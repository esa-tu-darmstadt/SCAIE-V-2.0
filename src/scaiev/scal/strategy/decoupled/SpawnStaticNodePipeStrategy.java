...
public class SpawnStaticNodePipeStrategy extends MultiNodeStrategy {
   ...
   */
  public SpawnStaticNodePipeStrategy(StrategyBuilders strategyBuilders, Verilog language, BNode bNodes, Core core,
                                     HashMap<SCAIEVNode, HashMap<String, PipelineStage>> spawn_instr_stage,
                                     HashMap<String, SCAIEVInstr> allISAXes) {
  ...
  private boolean implementSingle(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
    if (!nodeKey.getNode().tags.contains(NodeTypeTag.staticReadResult) || !nodeKey.getPurpose().matches(Purpose.PIPEDIN) ||
        nodeKey.getAux() != 0)
      return false;
    ...
    if (subpipelineISAX.isEmpty()) {
      subpipelineISAX = spawn_instr_stage.values()
                            .stream()
    ...
    if (subpipelineISAX.isEmpty()) {
      ...
      logger.warn("SpawnStaticNodePipeStrategy: Could not find an associated ISAX with sub-pipeline stage {}",
    ...
    if (nodeKey.getStage().getStagePos() == 0) {
      out.accept(NodeLogicBuilder.fromFunction("SpawnStaticNodePipeStrategy_fromParent_" + nodeKey.toString(false), (registry, aux) -> {
        ...
        String nodeValIn = registry.lookupExpressionRequired(
            new NodeInstanceDesc.Key(nodeKey.getNode(), nodeKey.getStage().getParent().get(), nodeKey.getISAX()), requestedFor);
        ret.logic += String.format("assign %s = %s;\n", signalName, nodeValIn);
        ret.outputs.add(
            new NodeInstanceDesc(new NodeInstanceDesc.Key(Purpose.PIPEDIN, nodeKey.getNode(), nodeKey.getStage(), nodeKey.getISAX()),
                                 signalName, ExpressionType.WireName, requestedFor));
    ...
    out.accept(NodeLogicBuilder.fromFunction("SpawnStaticNodePipeStrategy_" + nodeKey.toString(false), registry -> {
      ...
      for (PipelineStage prevStage : nodeKey.getStage().iterablePrev_bfs()) {
        ...
        prevValue_opt = registry.lookupOptionalUnique(prevValueKey, requestedFor).map(nodeInst -> nodeInst.getExpression());
        if (prevValue_opt.isPresent()) {
          prevValueStage_opt = Optional.of(prevStage);
          break;
        }
        if (prevStage.getPrev().size() > 1) {
          logger.warn("SpawnStaticNodePipeStrategy: Encountered sub-pipeline split into " + prevStage.getName() +
                      ". Deferring implementation to different Strategy.");
