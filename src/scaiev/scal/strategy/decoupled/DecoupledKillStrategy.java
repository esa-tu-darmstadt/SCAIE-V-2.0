...
public class DecoupledKillStrategy extends MultiNodeStrategy {
   ...
   */
  public DecoupledKillStrategy(Verilog language, BNode bNodes, Core core, HashMap<String, SCAIEVInstr> allISAXes) {
  ...
  public void implement(Consumer<NodeLogicBuilder> out, Iterable<NodeInstanceDesc.Key> nodeKeys, boolean isLast) {
    ...
    while (nodeKeyIter.hasNext()) {
      ...
      if ((nodeKey.getNode().equals(bNodes.RdIValid)) && nodeKey.getStage().getKind() == StageKind.Sub &&
        ...
        out.accept(NodeLogicBuilder.fromFunction("DecoupledKillStrategy_" + nodeKey.getStage().getName(), (registry, aux) -> {
          NodeInstanceDesc killCondInst = registry.lookupRequired(new NodeInstanceDesc.Key(
              DecoupledPipeStrategy.PseudoNode_StartSpawnToSpawnPipe_KillAll, nodeKey.getStage().getParent().get(), ""));
          ...
          ret.outputs.add(new NodeInstanceDesc(new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrFlush, nodeKey.getStage(), "", aux),
                                               killCondInst.getExpression(), ExpressionType.AnyExpression, killRequestedFor));
        ...
        out.accept(NodeLogicBuilder.fromFunction("DecoupledKillStrategy_RdKill_" + nodeKey.getStage().getName(), (registry, aux) -> {
          ...
          String killActiveCondition =
              startSpawnStages.asList()
                  ...
                  .reduce((a, b) -> a + " || " + b)
                  .orElse("1'b0");
          ret.outputs.add(new NodeInstanceDesc(new NodeInstanceDesc.Key(bNodes.RdKill, nodeKey.getStage(), nodeKey.getISAX()),
                                               killActiveCondition, ExpressionType.WireName, rdKillRequestedFor));
          return ret;
        }));
        nodeKeyIter.remove();
      }
      ...
      //		    			ISAXes.containsKey(stage_isax.getValue()) &&
      // ISAXes.get(stage_isax.getValue()).GetRunsAsDynamicDecoupled()
      //		    		)
      //		    		.distinct()
      //		    		.toList();
      ...
      //						.map(startSpawnStage ->
      //							registry.lookupExpressionRequired(new NodeInstanceDesc.Key(BNode.RdIValid,
      // startSpawnStage, SCAL.PredefInstr.kill.instr.GetName()))
      ...
      //					ret.outputs.add(new NodeInstanceDesc(new NodeInstanceDesc.Key(disaxkillCondNode,
      // core.GetRootStage(), ""), globalKillCondWire, ExpressionType.WireName)); 					for (var
      // stageISAXPair : relevantStageISAXPairs) { 						ret.outputs.add(new NodeInstanceDesc(new
      // NodeInstanceDesc.Key(BNode.ISAX_killReq, stageISAXPair.getKey(), stageISAXPair.getValue()), globalKillCondWire,
