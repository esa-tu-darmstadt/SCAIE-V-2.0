...
public interface NodeRegistryRO {
   ...
   *        Must not be partially-anonymized/-wildcarded, only {@link NodeRegistry#anonymizeKey(scaiev.scal.NodeInstanceDesc.Key)} is
   *        allowed.
   ...
   * @param key the key that identifies the node
   * @return the resolved node instance, or a placeholder with expression value starting with {@link NodeRegistry#MISSING_PREFIX}
   */
   ...
   * @return a new unique non-zero aux value.
   */
  int newUniqueAux();
   ...
   *        allowed.
   * @param fullMatch true to search for precise key matches including the ISAX and aux, false to ignore ISAX and aux
   * @param requestedFor the tracking set for relevant entities (e.g. ISAXes) that the key is being requested for; will be added to all
   *     returned nodes as a lazy reference
   * @return an Iterable over the node instances
   */
   ...
   *        Must not be partially-anonymized/-wildcarded, only {@link NodeRegistry#anonymizeKey(scaiev.scal.NodeInstanceDesc.Key)} is
   *        allowed.
   * @param requestedFor the tracking set for relevant entities (e.g. ISAXes) that the key is being requested for; will be added to the
   *     returned node as a lazy reference
   * @return the node instance wrapped in Optional (if unique), or an empty Optional
   */
   ...
   * @param key the key that identifies the node
   * @param requestedFor the tracking set for relevant entities (e.g. ISAXes) that the key is being requested for; will be added to the node
   *     as a lazy reference
   * @return the expression to retrieve the value of the given node (or a placeholder starting with {@link NodeRegistry#MISSING_PREFIX})
   */
   ...
   * @return the expression to retrieve the value of the given node (or a placeholder starting with {@link NodeRegistry#MISSING_PREFIX})
   */
  default String lookupExpressionRequired(NodeInstanceDesc.Key key) { return lookupRequired(key).expression; }
