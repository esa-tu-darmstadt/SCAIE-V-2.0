...
public class CoreBackend {
  public HashMap<SCAIEVNode, HashMap<PipelineStage, NodePropBackend>> nodes =
      new HashMap<SCAIEVNode, HashMap<PipelineStage, NodePropBackend>>();       // <NodeName,Module>
  ...
  public String getCorePathIn() { return null; }
  ...
  public int NodeSize(SCAIEVNode node, PipelineStage stage) { return node.size; }
  public boolean NodeIsInput(SCAIEVNode node, PipelineStage stage) { return node.isInput; }
  ...
  public String NodeDataT(SCAIEVNode node, PipelineStage stage) { return NodeProp(node, stage).map(entry -> entry.data_type).orElse(null); }
  ...
  public String NodeName(SCAIEVNode node, PipelineStage stage) { return NodeProp(node, stage).map(entry -> entry.name).orElse(null); }
  ...
  public String ModName(String module) { return Optional.ofNullable(fileHierarchy.get(module)).map(entry -> entry.name).orElse(null); }
  ...
  public String ModFile(String module) { return Optional.ofNullable(fileHierarchy.get(module)).map(entry -> entry.file).orElse(null); }
   ...
   */
  public List<MultiNodeStrategy> getAdditionalSCALPreStrategies() { return new ArrayList<MultiNodeStrategy>(); }
  /**
   * Add strategies to SCAL generation.
   * Run after all other strategies.
   */
  public List<MultiNodeStrategy> getAdditionalSCALPostStrategies() { return new ArrayList<MultiNodeStrategy>(); }

  /**
   * Add node into HashMap with corresponding Info. For signals that are typical for one core we won't define them in BNode, but pass their
   * name as string to PutNode.
   * @param size
   * @param input
   * @param data_type
   * @param assignSignal
   ...
   * @param stage
   */
  public void PutNode(int size, boolean input, String data_type, String assignSignal, String assignModule, String addNode,
                      PipelineStage stage) {
