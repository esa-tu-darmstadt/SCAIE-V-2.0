...
public class Piccolo extends CoreBackend {
  ...
  private int stageNum_spawn;
  private PipelineStage stage_spawn;
  ...
  private void IntegrateISAX_NoIllegalInstr() {
    ...
    this.toFile.UpdateContent(
        this.ModFile("fv_ALU"), "function ALU_Outputs fv_ALU (ALU_Inputs inputs);",
    ...
    if (this.op_stage_instr.containsKey(BNode.WrMem)) {
      toFile.ReplaceContent(this.ModFile("fv_ALU"), "alu_outputs.op_stage2 = OP_Stage2_ST;",
                            new ToWrite("alu_outputs.op_stage2 = is_isax ? OP_Stage2_ISAX :  OP_Stage2_ST;\n", false, true, ""));
      this.toFile.UpdateContent(
      ...
      this.toFile.UpdateContent(
          ...
          new ToWrite("let is_isax =  " +
                          language.CreateAllEncoding(op_stage_instr.get(BNode.WrMem).get(earliestWrMemStage), ISAXes, rdInstr) + ";\n",
                      false, true, ""));
      toFile.ReplaceContent(this.ModFile("fv_ALU"), "Bool legal_STORE = ",
                            new ToWrite("Bool legal_STORE = (   ((opcode == op_STORE) || is_isax)", false, true, ""));
      this.toFile.UpdateContent(
          this.ModFile("fv_ALU"), "if (inputs.decoded_instr.opcode ==",
          new ToWrite("if (%s) begin\n".formatted(language.CreateAllEncoding(op_stage_instr.get(BNode.WrMem).get(earliestWrMemStage), ISAXes, rdInstr))
                          + tab + "alu_outputs = fv_STORE (inputs);\n"
                          + "end\nelse",
                      true, false, "function ALU_Outputs fv_ALU", true));
    }
    if (this.op_stage_instr.containsKey(BNode.RdMem)) {
      toFile.ReplaceContent(this.ModFile("fv_ALU"), "alu_outputs.op_stage2 = OP_Stage2_LD;",
                            new ToWrite("alu_outputs.op_stage2 = is_isax ? OP_Stage2_ISAX :  OP_Stage2_LD;\n", false, true, ""));
      ...
      this.toFile.UpdateContent(
          this.ModFile("fv_ALU"), "function ALU_Outputs fv_LOAD (ALU_Inputs inputs);",
          new ToWrite("let is_isax =  " +
                          language.CreateAllEncoding(op_stage_instr.get(BNode.RdMem).get(earliestRdMemStage), ISAXes, rdInstr) + ";\n",
                      false, true, ""));
      toFile.ReplaceContent(this.ModFile("fv_ALU"),
                            "Bool legal_LOAD = ", new ToWrite("Bool legal_LOAD = (   ((opcode == op_LOAD) || is_isax)", false, true, ""));

      this.toFile.UpdateContent(
          this.ModFile("fv_ALU"), "if (inputs.decoded_instr.opcode ==",
          new ToWrite("if (%s) begin\n".formatted(language.CreateAllEncoding(op_stage_instr.get(BNode.RdMem).get(earliestRdMemStage), ISAXes, rdInstr))
                          + tab + "alu_outputs = fv_LOAD (inputs);\n"
                          + "end\nelse",
                      true, false, "function ALU_Outputs fv_ALU", true));
    }
    if (this.op_stage_instr.containsKey(BNode.RdRS1) || this.op_stage_instr.containsKey(BNode.RdInstr) ||
        this.op_stage_instr.containsKey(BNode.WrPC) || this.op_stage_instr.containsKey(BNode.RdPC) ||
        this.op_stage_instr.containsKey(BNode.RdIValid)) {
      String ifClause = language.CreateAllNoMemEncoding(allISAX, ISAXes, rdInstr);
      if (!ifClause.isEmpty()) {
        this.toFile.UpdateContent(
            this.ModFile("fv_ALU"), "if (inputs.decoded_instr.opcode ==",
            new ToWrite("if (%s) begin\n".formatted(language.CreateAllNoMemEncoding(allISAX, ISAXes, rdInstr))
                            + tab + "alu_outputs = alu_outputs_base;\n"
                            + tab + "alu_outputs.op_stage2 = OP_Stage2_ISAX;\n"
                            + "end\nelse",
                        true, false, "function ALU_Outputs fv_ALU", true));
      }
    }
    this.toFile.UpdateContent(this.ModFile("CPU_Globals"), "typedef enum {  OP_Stage2_ALU",
                              new ToWrite(", OP_Stage2_ISAX", false, true, "")); // leave it heere outside of if-else to avid undefined err
  }
  ...
  private void IntegrateISAX_WrRD() {
    ...
    if (op_stage_instr.containsKey(BNode.WrRD)) {
      String mkCPU_validDataStage1Expr =
          (this.ContainsOpInStage(BNode.WrRD_validData, 1) ? language.CreateLocalNodeName(BNode.WrRD_validData, stages[1], "") : "False");
      if (this.ContainsOpInStage(BNode.WrRD, 2)) {

        // Valid bit
        toFile.UpdateContent(this.ModFile("mkCPU_Stage3"), "// BEHAVIOR",
                             new ToWrite("let isax_rg_stage3_rd_valid = (" + language.CreateLocalNodeName(BNode.WrRD_validData, stage, "") +
                                             ") ? " + language.CreateLocalNodeName(BNode.WrRD_valid, stage, "") +
                                             " : rg_stage3.rd_valid;\n",
                                         false, true, ""));
        ...
        /// Data
        toFile.UpdateContent(this.ModFile("mkCPU_Stage3"), "// BEHAVIOR",
                             new ToWrite("let isax_rg_stage3_rd_val = ((" + language.CreateLocalNodeName(BNode.WrRD_validData, stage, "") +
                                             " && (!" + language.CreateLocalNodeName(BNode.WrRD_validData, stages[stageNum - 1], "") +
                                             ")) ? " + language.CreateLocalNodeName(BNode.WrRD, stage, "") + " : rg_stage3.rd_val);\n",
                                         false, true, ""));
        toFile.ReplaceContent(this.ModFile("mkCPU_Stage3"),
                              "rd_val:", new ToWrite("rd_val:       isax_rg_stage3_rd_val\n", true, false, "let bypass_base = Bypass"));
        toFile.ReplaceContent(this.ModFile("mkCPU_Stage3"), "gpr_regfile.write_rd",
                              new ToWrite("gpr_regfile.write_rd (rg_stage3.rd, isax_rg_stage3_rd_val);\n", true, false, "// Write to GPR"));
        toFile.ReplaceContent(this.ModFile("mkCPU_Stage3"), "rg_stage3.rd, rg_stage3.rd_val",
                              new ToWrite("rg_stage3.rd, isax_rg_stage3_rd_val);\n", true, false, "write GRd 0x"));
        toFile.ReplaceContent(this.ModFile("mkCPU_Stage3"), "if (rg_stage3.rd_valid) begin",
                              new ToWrite("if (isax_rg_stage3_rd_valid) begin", false, true, ""));
        toFile.ReplaceContent(
            this.ModFile("mkCPU_Stage3"), "bypass.bypass_state = (rg_full && rg_stage3.rd_valid)",
            new ToWrite(" bypass.bypass_state = (rg_full && isax_rg_stage3_rd_valid) ? BYPASS_RD_RDVAL", false, true, ""));
        toFile.ReplaceContent(
            this.ModFile("mkCPU_Stage3"), "bypass.bypass_state = (rg_full && rg_stage3.rd_valid)",
            new ToWrite(" bypass.bypass_state = (rg_full && isax_rg_stage3_rd_valid) ? BYPASS_RD_RDVAL", true, false, "`else"));
        toFile.ReplaceContent(
            this.ModFile("mkCPU_Stage3"), "bypass.bypass_state = (rg_full && rg_stage3.rd_valid)",
            new ToWrite(" bypass.bypass_state = (rg_full && isax_rg_stage3_rd_valid) ? BYPASS_RD_RDVAL", true, false, "else begin"));

        //				// If not present lso in stage 1, make sure validreq is
        //				if(!this.ContainsOpInStage(BNode.WrRD, 1))
        //					language.UpdateInterface(this.ModFile("mkCPU_Stage2"),BNode.WrRD_valid,
        //"",stages[1],true,false);

      } else if (this.ContainsOpInStage(BNode.WrRD, 1)) {
        //				if(!this.ContainsOpInStage(BNode.WrRD, 2))
        //					language.UpdateInterface(this.ModFile("mkCPU"),BNode.WrRD_valid, "",stages[2],true,false);
        //// Just to avoid wrapper cry that this signal does not exist. Will be optimized away anyway
      }
      ...
      if (this.ContainsOpInStage(BNode.WrRD, 2) || this.ContainsOpInStage(BNode.WrRD, 1)) {
        ...
        //				if(!this.ContainsOpInStage(BNode.WrRD, 1)) // if it doesn't contain it to be generated
        // automatically, add it artifically for Data Hazard logic 					language.UpdateInterface("mkCore",
        // BNode.WrRD_validData, "", stages[1], true, false); // Add validData stage 1 on top interface
        // if(!this.ContainsOpInStage(BNode.WrRD, 2)) // If it does not contain it, add it artificially for top module wrapper
        // language.UpdateInterface("mkCore", BNode.WrRD_validData, "", stages[2], true, false);
        //// Add validData stage 2 on top interface, although not used!! Just for script top module

        if (this.ContainsOpInStage(BNode.WrRD, 1)) // Only if we have a WrRD in stage 1 it makes sense to check for WrRD_validData
          addText += " else if (" + language.CreateLocalNodeName(BNode.WrRD_valid, stages[1], "") + " && " +
                     language.CreateLocalNodeName(BNode.WrRD_validData, stages[1], "") + ") begin\n" +
        ...
        // this.toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "// This stage is just relaying ALU", new ToWrite(addText, false, true ,
        // "", true ));
        this.toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "return output_stage2;", new ToWrite(addText, false, true, "", true));
      }
    }

    // No Bypass or data to write to RegF if: 1) I have a WrRD with user valid that can be canceled 2) I have at least 1 instr not writing
    // no regfile
    ...
    if (!this.op_stage_instr.containsKey(BNode.WrRD))
      ...
      for (String ISAX : ISAXes.keySet()) {
        ...
        if (ISAXes.get(ISAX).GetSchedNodes().containsKey(BNode.WrRD) &&
            ISAXes.get(ISAX).GetSchedNodes().get(BNode.WrRD).get(0).HasAdjSig(AdjacentNode.validReq)) // TODO list , we should get all
          noWrRD = true;
      }
    }
    if (noWrRD) {
      // this.toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "// This stage is just relaying ALU", new ToWrite("else
      // if(rg_stage2.op_stage2 ==  OP_Stage2_ISAX ) begin\n"+CreateMemStageFrwrd(false,false,false),false,true,"", true));
      this.toFile.UpdateContent(
          this.ModFile("mkCPU_Stage2"), "return output_stage2;",
          new ToWrite("else if(rg_stage2.op_stage2 ==  OP_Stage2_ISAX ) begin\n" + CreateMemStageFrwrd(false, false, false), false, true,
                      "", true));
    }
  }
  ...
  //		this.toFile.UpdateContent(this.ModInterfFile("mkCPU"), "endinterface",new
  // ToWrite(methodDecl+";\n",true,false,"interface",true)); 		this.toFile.UpdateContent(this.ModInterfFile("mkCore"),
  // "endinterface",new ToWrite("(*always_enabled *)"+methodDecl+";\n",true,false,"interface",true)); 		String assignText =
  // methodDecl+";\n"+tab+"return
  //"+assignTo+";\nendmethod"; 		this.toFile.UpdateContent(this.ModFile("mkCPU"), "endmodule",new
  // ToWrite(assignText,true,false,"module mkCPU",true)); 		String methodDeclShort = language.CreateMethodDecl(operation, stage,
  // "", true); 		this.toFile.UpdateContent(this.ModFile("mkCore"), "endmodule",new ToWrite(methodDeclShort+" =
  // cpu."+methodName+";\n",true,false,"module mkCore",true));
  //	}

  /**
   * Additional stall conditions to apply to the given stage. Should also be returned through RdStall.
   * @return String of individual conditions, to be ORed together
   */
  private Stream<String> additionalStallConds(int stageNum) {
    Stream<String> extraConds = Stream.empty();
    if (stageNum == 2) {
      if (ContainsOpInStage(BNode.WrRD_spawn, stage_spawn))
        extraConds = Stream.concat(extraConds, Stream.of("stage3.has_WrRD_spawn_valid()"));
    }
    return extraConds;
  }
  private void IntegrateISAX_RdStall() {
    for (int stageNum = 0; stageNum <= 2; stageNum++) {
      ...
      //Create a Bool within the rule.
      String ruleLocalRdStall = "ruleRdStall_%d".formatted(stageNum);
      toFile.UpdateContent(this.ModFile("mkCPU"), ");",
          new ToWrite("Bool %s = True;\n".formatted(ruleLocalRdStall), true, false, "rule rl_pipe"));

      if (stageNum == 1) {
        toFile.UpdateContent(this.ModFile("mkCPU"), "end",
                             new ToWrite("else\n" + tab + ruleLocalRdStall + " = True;", true, false,
                                         "// Move instruction from Stage" + (stageNum + 1) + " to Stage" + (stageNum + 2)));
        toFile.UpdateContent(this.ModFile("mkCPU"), "stage3.enq (stage2.out.data_to_stage3);",
                             new ToWrite(ruleLocalRdStall + " = False;", false, true, ""));
      } else if (stageNum == 0) {
        toFile.UpdateContent(this.ModFile("mkCPU"), "begin",
                             new ToWrite(ruleLocalRdStall + " = False;", true, false,
                                         "// Move instruction from Stage" + (stageNum + 1) + " to Stage" + (stageNum + 2)));
        toFile.UpdateContent(this.ModFile("mkCPU"), "end",
                             new ToWrite("else\n" + tab + ruleLocalRdStall + " = True;", true, false,
                                         "// Move instruction from Stage" + (stageNum + 1) + " to Stage" + (stageNum + 2)));
      } else if (stageNum == 2) {
        String toAdd = ruleLocalRdStall + " = stage3.out.ostatus != OSTATUS_PIPE;";
      ...
      String extraConds = additionalStallConds(stageNum).reduce((a,b)->a+" || "+b).orElse("");
      if (!extraConds.isEmpty()) {
        //Check for other RdStall reasons
        toFile.UpdateContent(this.ModFile("mkCPU"), "endrule: rl_pipe",
                             new ToWrite("if (%s)\n%s%s = True;".formatted(extraConds, tab, ruleLocalRdStall), false, true, "", true));
      }
      //Set module-local stall wire from rule-local one
      toFile.UpdateContent(this.ModFile("mkCPU"), "endrule: rl_pipe",
                           new ToWrite("%s <= %s;".formatted(localRdStallWire, ruleLocalRdStall), false, true, "", true));
  ...
  private void IntegrateISAX_WrStall() {
    String[] wrstallConds = new String[3];
    for (int iStage = 0; iStage < 3; ++iStage) {
      Stream<String> conds_stage = additionalStallConds(iStage);
      if (ContainsOpInStage(BNode.WrStall, iStage))
        conds_stage = Stream.concat(Stream.of(language.CreateLocalNodeName(BNode.WrStall, stages[iStage], "")), conds_stage);
      wrstallConds[iStage] = conds_stage.reduce((a,b)->a+" || "+b).orElse("");
    }

    if (!wrstallConds[1].isEmpty())
      toFile.ReplaceContent(this.ModFile("mkCPU"), "if ((! stage3_full)",
                            new ToWrite("if ((! stage3_full) && (stage2.out.ostatus == OSTATUS_PIPE) && !(" +
                                            wrstallConds[1] + ")) begin\n",
                                        true, false, "// Move instruction from Stage2 to Stage3"));
    if (!wrstallConds[0].isEmpty())
      toFile.ReplaceContent(this.ModFile("mkCPU"), "if (   (! halting)",
                            new ToWrite("if (   (! halting) && !(" + wrstallConds[0] + ")", true,
                                        false, "// Move instruction from Stage1 to Stage2"));
    //Need to stall entire pipeline on WrStall_2:
    // rl_pipe uses stage3.deq (inside a conditional statement),
    //   which needs regfile access -> conflict with the WrRD_spawn rule in stage3.
    if (!wrstallConds[2].isEmpty())
      toFile.ReplaceContent(this.ModFile("mkCPU"), "rule rl_pipe",
                            new ToWrite("rule rl_pipe (   (rg_state == CPU_RUNNING) && (!(" + wrstallConds[2] + "))",
                                        false, true, ""));
//    if (!wrstallConds[2].isEmpty())
//      toFile.ReplaceContent(this.ModFile("mkCPU"), "if (stage3.out.ostatus == OSTATUS_PIPE) begin",
//                            new ToWrite("if (stage3.out.ostatus == OSTATUS_PIPE && !(%s)) begin".formatted(wrstallConds[2]),
//                                        false, true, ""));
  }
  private void IntegrateISAX_WrPC() {
    ...
    for (int stageNum = this.piccolo_core.maxStage; stageNum >= 0; stageNum--) {
      ...
      if (stageNum == 0)
        if (op_stage_instr.containsKey(BNode.WrPC_spawn)) {
          updatePC += "(" + language.CreateLocalNodeName(BNode.WrPC_spawn_valid, stages[this.piccolo_core.maxStage + 1], "") + ") ? (" +
                      language.CreateLocalNodeName(BNode.WrPC_spawn, stages[this.piccolo_core.maxStage + 1], "") + ") : (";
          nrJumps++;
        }

      if (ContainsOpInStage(BNode.WrPC, stageNum)) {
        updatePC += "(" + language.CreateLocalNodeName(BNode.WrPC_valid, stage, "") + ") ? (" +
                    language.CreateLocalNodeName(BNode.WrPC, stage, "") + ") : (";
        nrJumps++;
      }
    }
    ...
    if (nrJumps > 0) {
      updatePC += "next_pc"
                  + ")".repeat(nrJumps) + ";";
      toFile.ReplaceContent(this.ModFile("mkCPU_Stage1"), "output_stage1.next_pc",
                            new ToWrite("output_stage1.next_pc = isax_next_pc;", true, false, "let next_pc"));
      toFile.UpdateContent(this.ModFile("mkCPU_Stage1"), ": fall_through_pc);",
                           new ToWrite("let isax_next_pc =" + updatePC, true, false, "let next_pc"));
    }
  }
  ...
  private void IntegrateISAX_RdFlush() {
    if (ContainsOpInStage(BNode.RdFlush, 1)) {
      ...
      toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "(CPU_Stage2_IFC);",
                           new ToWrite("Wire #( Bool ) " + localFlushWire + " <- mkDWire( False );\n", true, false, "module mkCPU_Stage2"));

      // Within function fv_out:
      toFile.ReplaceContent(this.ModFile("mkCPU_Stage2"), "function Output_Stage2 fv_out;",
                            new ToWrite("function Tuple2#(Output_Stage2,Bool) fv_out;\n", false, true, ""));
      toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "Output_Stage2 output_stage2 = ?;",
                           new ToWrite("Bool flushing = False;\n", false, true, ""));
      ...
      toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), ";",
                           new ToWrite("flushing = ostatus == OSTATUS_NONPIPE;\n", true, false, "? OSTATUS_NONPIPE"));
      toFile.ReplaceContent(this.ModFile("mkCPU_Stage2"), "return output_stage2;",
                            new ToWrite("return tuple2(output_stage2, flushing);\n", false, true, ""));
      ...
      toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "endfunction",
                           new ToWrite("\n"
    ...
    if (op_stage_instr.containsKey(BNode.WrFlush)) {
      ...
      if (!flushCond.isEmpty()) {
        ...
        toFile.UpdateContent(
            this.ModFile("mkCPU_Stage2"), grep,
            new ToWrite("if(" + validPCFlush + ") begin \n " + CreateMemStageFrwrd(false, true, false), false, true, "", true));
      }
  ...
  private void IntegrateISAX_SpawnRD() {
    ...
    if (op_stage_instr.containsKey(BNode.WrRD_spawn)) {
      // Update stage 3
      String stage3 = "rule rule_" + language.CreateNodeName(BNode.WrRD_spawn_valid, stage_spawn, "") + "(" +
                      language.CreateLocalNodeName(BNode.WrRD_spawn_valid, stage_spawn, "") + ");\n" + tab + "if(" +
                      language.CreateLocalNodeName(BNode.WrRD_spawn_valid, stage_spawn, "") + ")\n" + tab + "gpr_regfile.write_rd (" +
                      language.CreateLocalNodeName(BNode.WrRD_spawn_addr, stage_spawn, "") + ", " +
                      language.CreateLocalNodeName(BNode.WrRD_spawn, stage_spawn, "") + ");\n"
                      + "endrule\n";
      this.toFile.UpdateContent(this.ModFile("mkCPU_Stage3"), grepDeclareBehav, new ToWrite(stage3 + "\n", false, true, "", true));

      String hasSpawnMethodImpl = "method Bool has_WrRD_spawn_valid = %s;"
          .formatted(language.CreateLocalNodeName(BNode.WrRD_spawn_valid, stage_spawn, ""));
      this.toFile.UpdateContent(this.ModFile("mkCPU_Stage3"), grepDeclareBehav, new ToWrite(hasSpawnMethodImpl + "\n", false, true, "", true));
      this.toFile.UpdateContent(this.ModFile("mkCPU_Stage3"), "endinterface", new ToWrite("(* always_ready *) method Bool has_WrRD_spawn_valid;",
                                                                                          true, false, "interface CPU_Stage3_IFC;", true));
    }
  }
  ...
  private void IntegrateISAX_Mem() {
    ...
    if (rdMem || wrMem) {
      ...
      if (wrMem) {
        ...
        toAdd = "else if (x.op_stage2 == OP_Stage2_ST || " + language.CreateLocalNodeName(BNode.WrMem_validReq, stage, "") +
                ")  cache_op = CACHE_ST;\n";
      ...
      if (rdMem) {
        ...
        toAdd = "if      (x.op_stage2 == OP_Stage2_LD || " + language.CreateLocalNodeName(BNode.RdMem_validReq, stage, "") +
                ")  cache_op = CACHE_LD;\n";
        this.toFile.ReplaceContent(this.ModFile("mkCPU_Stage2"), toGrep, new ToWrite(toAdd, false, true, ""));
      }
      toGrep = "if ((x.op_stage2 == OP_Stage2_LD) || (x.op_stage2 == OP_Stage2_ST)";
      toAdd = "if ((x.op_stage2 == OP_Stage2_LD) || (x.op_stage2 == OP_Stage2_ST) || op_stage2_amo || (x.op_stage2 == OP_Stage2_ISAX && (" +
              validMem + ") ) ) begin\n";
      this.toFile.ReplaceContent(this.ModFile("mkCPU_Stage2"), toGrep, new ToWrite(toAdd, false, true, ""));
      ...
      toAdd = "else if (rg_stage2.op_stage2 == OP_Stage2_ISAX && (! dcache.valid || dcache.exc) && isax_mem) begin\n"
              ...
              + "					};\n"
              + "end \n";
      this.toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "// This stage is just relaying ALU",
                                new ToWrite(toAdd, false, true, "", true));

      // Wdata
      if (wrMem) {
        this.toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "dcache.req",
                                  new ToWrite("let iasx_wdata_from_gpr = (" +
                                                  language.CreateLocalNodeName(BNode.WrMem_validReq, stage, "") + ") ? zeroExtend (" +
                                                  language.CreateLocalNodeName(BNode.WrMem, stage, "") + ") : wdata_from_gpr;",
                                              false, true, "", true));
        this.toFile.ReplaceContent(this.ModFile("mkCPU_Stage2"), "wdata_from_gpr,",
                                   new ToWrite("iasx_wdata_from_gpr,", true, false, "dcache.req"));
      }

      // Address
      String addr = "";
      if (rdMem) {
        addr += language.CreateLocalNodeName(BNode.RdMem_addr_valid, stage, "") + " ? " +
                language.CreateLocalNodeName(BNode.RdMem_addr, stage, "") + " : ";
      }
      if (wrMem)
        addr = language.OpIfNEmpty(addr, "( ") + language.CreateLocalNodeName(BNode.WrMem_addr_valid, stage, "") + " ? " +
               language.CreateLocalNodeName(BNode.WrMem_addr, stage, "") + " : ";
      addr += "  x.addr ";
      if (rdMem && wrMem)
        addr += ")";
      this.toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "dcache.req",
                                new ToWrite("let isax_x_addr = " + addr + ";", false, true, "", true));
  ...
  private void IntegrateISAX_SpawnMem() {
    ...
    if (op_stage_instr.containsKey(BNode.RdMem_spawn) || op_stage_instr.containsKey(BNode.WrMem_spawn)) {
      ...
      this.toFile.UpdateContent(this.ModFile("mkCPU_Stage2"), "let funct3 = instr_funct3 (x.instr);",
      ...
      String writeRes = "rule rule_send_to_stage2_spawn( " + language.CreateLocalNodeName(BNode.WrMem_spawn_validReq, spawnStage, "") +
                        " &&  !isax_memONgoing);\n" + tab + "stage2.met_commitspawn (" +
                        language.CreateLocalNodeName(BNode.RdMem_spawn_addr, spawnStage, "") + "," + writeData + "," +
                        language.CreateLocalNodeName(BNode.RdMem_spawn_validReq, spawnStage, "") + "," +
                        language.CreateLocalNodeName(BNode.RdMem_spawn_write, spawnStage, "") + ");\n" + tab + "if(" +
                        language.CreateLocalNodeName(BNode.RdMem_spawn_validReq, spawnStage, "") + ") isax_memONgoing <= True; \n"
                        + "endrule\n";
      ...
      this.toFile.UpdateContent(
          this.ModFile("mkCPU_Stage2"), "endinterface",
          new ToWrite("method Action met_commitspawn (Bit#(32) x,Bit#(32) y,Bool z, Bool read);\n", false, true, "", true));
      ...
      String checkSpawnDone = "rule rule_isaxwire;\n"
                              ...
                              + "\n";
      this.toFile.UpdateContent(
          this.ModFile("mkCPU"), "module mkCPU",
          new ToWrite("Wire #(Bool) isax_memONgoing_wire <- mkDWire(False);\nReg #(Bool) isax_memONgoing <- mkReg(False);\n", false, true,
                      ""));
  ...
  private boolean ContainsOpInStage(SCAIEVNode op, int stage) { return ContainsOpInStage(op, stages[stage]); }
  ...
  private void ConfigPiccolo() {
    ...
    this.PutNode("Bool", "(stage1.out.ostatus == OSTATUS_NONPIPE && stage1.out.control == CONTROL_TRAP)" + wrFlush1Cond, "mkCPU",
                 BNode.RdFlush, stages[0]);
    ...
    this.PutNode("Bool", "True", "mkCPU", BNode.WrRD_spawn_validResp, stages[spawnStage]);
    ...
    this.PutNode("Bool", "True", "mkCPU", BNode.ISAX_spawnAllowed, stages[0]);
  }
}
