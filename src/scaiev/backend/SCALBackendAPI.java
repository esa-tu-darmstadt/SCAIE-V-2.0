...
public interface SCALBackendAPI {
  ...
  public static class CustomCoreInterface {
    ...
    public SCAIEVNode makeNode(boolean forCoreSide) { return new SCAIEVNode(name, size, isInputToSCAL ^ forCoreSide); }
    public NodeInstanceDesc.Key makeKey(Purpose purpose) { return new NodeInstanceDesc.Key(purpose, makeNode(false), stage, instr_name); }
  ...
  /**
   * Some nodes like Memory require the Valid bit in earlier stages. In case of Memory for exp. for computing dest. address. SCAL will
   * generate these valid bits, but SCAL must know from which stage they are requried. These valid bits are not combined with the optional
   * user valid bit. They are just based on instr decoding Function to be overwritten by cores which need this.
   * @param node ...node
