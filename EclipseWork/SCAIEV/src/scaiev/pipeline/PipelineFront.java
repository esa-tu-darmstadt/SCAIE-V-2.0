 ...
 *   Thus, {@code new PipelineFront(stage.parent).isAround(stage)} is true while {@code new PipelineFront(stage).isAround(stage.parent)} is not.
 *   There is no before/after ordering between parents and children.
 * <p>
...
public class PipelineFront {
	...
	public List<PipelineStage> asList() {
		return Collections.unmodifiableList(frontList);
	}
	 ...
	 * @param queryStage
	 */
	public boolean contains(PipelineStage queryStage) {
		return front.contains(queryStage);
	}
	...
	public boolean isBefore(PipelineStage queryStage, boolean needs_continuous) {
		...
		return queryStage.prev.stream().anyMatch(prevStage -> (
			this.isAroundOrBefore(prevStage, needs_continuous) //Test the previous stage
			//Test the child tails of the previous stage (note: if prevStage does have children, the check below will overlap with the one above)
	...
	public boolean isAfter(PipelineStage queryStage, boolean needs_continuous) {
		...
		//Simple recursive implementation. Performance should not be too much of a concern.
		return nextStream.anyMatch(nextStage -> 
			this.isAroundOrAfter(nextStage, needs_continuous) //Test the next stage
			//Test the children of the next stage (note: if nextStage does have children, the check below will overlap with the one above)
	 ...
	 */
	public Stream<PipelineStage> streamNext_bfs() {
		return streamNext_bfs(stage_ -> true);
	}
	/**
	 ...
	 */
	public Stream<PipelineStage> streamPrev_bfs() {
		return streamPrev_bfs(stage_ -> true);
	}
}
