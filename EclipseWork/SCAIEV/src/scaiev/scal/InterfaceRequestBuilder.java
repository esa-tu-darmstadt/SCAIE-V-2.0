...
public class InterfaceRequestBuilder extends NodeLogicBuilder {
	...
	/**
	 * Apply the InterfaceRequestBuilder, but with the third parameter specifying if the interface pin is a required or an optional dependency.
	 */
	public NodeLogicBlock applyOptional(NodeRegistryRO registry, int aux, boolean optional) {
		...
		Optional<NodeInstanceDesc> markerInst_opt = optional ? registry.lookupOptionalUnique(nodeKey) : Optional.of(registry.lookupRequired(lookupMarkerKey));
