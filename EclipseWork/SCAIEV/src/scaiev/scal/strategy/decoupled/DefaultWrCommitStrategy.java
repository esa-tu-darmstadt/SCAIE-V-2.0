...
public class DefaultWrCommitStrategy extends SingleNodeStrategy {
	...
	public Optional<NodeLogicBuilder> implement(NodeInstanceDesc.Key nodeKey) {
		if (nodeKey.getPurpose().matches(Purpose.WIREDIN_FALLBACK)
				&& nodeKey.getNode().equals(bNodes.WrCommit_spawn)
			...
			return Optional.of(NodeLogicBuilder.fromFunction("DefaultWrCommitStrategy_"+nodeKey.toString(), registry -> {
				...
				String validReqExpr = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(Purpose.match_WIREDIN_OR_PIPEDIN, bNodes.WrCommit_spawn_validReq, nodeKey.getStage(), nodeKey.getISAX()));
				ret.logic += String.format("assign %s = %s;\n", wireName, validReqExpr);
				ret.outputs.add(new NodeInstanceDesc(
					new NodeInstanceDesc.Key(Purpose.WIREDIN_FALLBACK, bNodes.WrCommit_spawn, nodeKey.getStage(), nodeKey.getISAX()),
					wireName,
