...
public class DefaultRdwrInStageStrategy extends MultiNodeStrategy {
	...
	private static final Purpose DefaultRdwrInStageInternalPurpose = new Purpose("DefaultRdwrInStage-Internal", true, Optional.empty(), List.of());
		...
		if (!coreSupportsDeqInstrInStage && implementedKeys.add(wrDeqInstrKey)) {
			...
			out.accept(NodeLogicBuilder.fromFunction("DefaultRdwrInStageStrategy_WrDeqInstr_"+stage.getName(), (registry, aux) -> {
				...
				ret.outputs.add(new NodeInstanceDesc(
					NodeInstanceDesc.Key.keyWithPurpose(wrDeqInstrKey, DefaultRdwrInStageInternalPurpose),
				...
				stallCondBody += String.format("%s = %s /*comb. new instruction dequeue*/ || %s && !%s /*still pending sub-pipeline instruction*/;\n",
					pendingInstrStallWire,
					deqInstrExpr,
					instrHiddenReg, wrInStageExpr
				);
				...
				stallCondBody += tab+String.format("$display(\"ERROR: Overriding retire in stage %s: Cannot perform WrInStageID without a corresponding WrDeqInstr\");\n", stage.getName());
				stallCondBody += tab+"$stop;\n";
				stallCondBody += "end\n";
				ret.logic += language.CreateInAlways(false, stallCondBody);
				
				ret.outputs.add(new NodeInstanceDesc(new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrStall, stage, "", aux), pendingInstrStallWire, ExpressionType.WireName, wrDeqRequestedFor));
