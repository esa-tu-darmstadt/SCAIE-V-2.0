...
public class SpawnOutputSelectStrategy extends MultiNodeStrategy {
	...
	private void LogicToCoreSpawn(NodeLogicBlock logicBlock, NodeRegistryRO registry, RequestedForSet requestedFor, SCAIEVNode node, PipelineStage stage) {
		...
		SpawnFireStrategy.getPriorityValidReqKeysStream(Purpose.REGISTERED, bNodes, op_stage_instr, stage, 
				SpawnFireStrategy.getISAXPriority(bNodes, isaxesSortedByPriority, mainNode))
			.forEachOrdered(curValidReqKeys -> {
				String curValidReq = curValidReqKeys.stream().map(key -> registry.lookupExpressionRequired(key)).reduce((a,b)->a+" || "+b).orElse("1'b0");
				...
				Optional<SCAIEVNode> selectedNode = 
					node.isAdj() 
					? bNodes.GetAdjSCAIEVNode(selectedMainNode, node.getAdj()) 
					: Optional.of(selectedMainNode);
				if (selectedNode.isPresent()) {
					...
					if (selectedNode.get().getAdj() == AdjacentNode.isWrite) {
						...
						String curNodeKey = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(
							Purpose.REGISTERED, selectedNode.get(), stage, curValidReqKeys.get(0).getISAX()
						));
		...
		if (!body.isEmpty()) {
			...
			logicBlock.declarations += (nodeToCore.size > 0)
				? String.format("reg [%d-1:0] %s;\n", nodeToCore.size, toCoreSignalName)
				: String.format("reg %s;\n", toCoreSignalName);
			logicBlock.outputs.add(new NodeInstanceDesc(
				new NodeInstanceDesc.Key(nodeToCore, stage, ""),
				toCoreSignalName,
	...
	private void LogicToISAXSpawnAdj(NodeLogicBlock logicBlock, NodeRegistryRO registry, SCAIEVNode node, PipelineStage stage, String ISAX) {
		...
		logicBlock.logic += String.format("assign %s = %s && !(%s) && %s && %s;\n",
			toISAXSignalName,
			registry.lookupExpressionRequired(new NodeInstanceDesc.Key(SpawnFireStrategy.ISAX_fire2_r, stage, SpawnFireStrategy.getFireNodeSuffix(node))),
			priority,
			registry.lookupExpressionRequired(new NodeInstanceDesc.Key(Purpose.REGISTERED, bNodes.GetAdjSCAIEVNode(mainNode, AdjacentNode.validReq).get(), stage, ISAX)),
			...
			if (nodeKey.getISAX().isEmpty() && nodeKey.getNode().isInput) {
				...
				keyIter.remove();
					...
					&& bNodes.GetEquivalentNonspawnNode(nodeKey.getNode())
						.map(nonspawnNode -> !nonspawnNode.tags.contains(NodeTypeTag.defaultNotprovidedByCore) || core.GetNodes().containsKey(nonspawnNode))
						.orElse(false)) {
				...
				if (builtToCoreNodeNameSet.add(builderIdentifier)) {
					...
					out.accept(NodeLogicBuilder.fromFunction("SpawnOutputSelectStrategy_nonspawnToSpawn_"+builderIdentifier, registry -> {
						...
						NodeInstanceDesc nodespawnNodeInst = registry.lookupRequired(new NodeInstanceDesc.Key(nodeNonspawnAdj, nodeKey.getStage(), ""), requestedFor);
						...
						ret.outputs.add(new NodeInstanceDesc(NodeInstanceDesc.Key.keyWithPurpose(nodeKey, Purpose.REGULAR), wireName, ExpressionType.WireName, requestedFor));
					...
					out.accept(NodeLogicBuilder.fromFunction("SpawnOutputSelectStrategy_toISAX_"+builderIdentifier, registry -> {
						...
						if (nodeKey.getNode().DefaultMandatoryAdjSig()) {
							if (nodeKey.getNode().size != 1) {
								logger.error("SpawnOutputSelectStrategy: Encountered node " + nodeKey.getNode().name + " with size " + nodeKey.getNode().size + " (expected size 1).");
								return ret;
							}
							if (nodeKey.getNode().getAdj() != AdjacentNode.validResp) {
								...
								logger.error("SpawnOutputSelectStrategy: Encountered 'mandatory adj' node " + nodeKey.getNode().name + " (not supported yet).");
								return ret;
							}
