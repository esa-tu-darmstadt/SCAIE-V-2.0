...
public class SpawnRdIValidStrategy extends MultiNodeStrategy {
	 ...
	 * Note: The width and max value of a particular counter NodeInstanceDesc can be retrieved via getKey().getNode().{size,elements} respectively.
	 */
	 ...
	 */
	public SpawnRdIValidStrategy(StrategyBuilders strategyBuilders,
	...
	private List<PipelineStage> getEndStagesFor(PipelineStage stage, String isaxName) {
		return spawn_instr_stage.entrySet().stream()
	...
	private boolean implementCounter(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
		if (!nodeKey.getPurpose().matches(Purpose.REGULAR) || nodeKey.getISAX().isEmpty() || nodeKey.getAux() != 0)
			return false;
		if (nodeKey.getNode().equals(ISAXValidCounter) || nodeKey.getNode().equals(ISAXValidCounterMax) || nodeKey.getNode().equals(ISAXValidCounterWidth)) {
			...
			if (implementedCounters.add(new NodeInstanceDesc.Key(ISAXValidCounter, nodeKey.getStage(), nodeKey.getISAX()))) {
				...
				out.accept(NodeLogicBuilder.fromFunction("SpawnRdIValidStrategy_"+counterOutputKey.toString(false), (registry, aux) -> {
					...
					if (counterMaxStr.equals("0")) {
						...
						String issueCond = String.format("%s && %s",
						...
						String commitCond = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.WrCommit_spawn_validReq, nodeKey.getStage(), nodeKey.getISAX()));

						//Assuming only the entire ISAX can be flushed, and the flush is visible on the end stages of all sub-pipelines.
						List<String> flushConds = endStages.stream().map(endStage -> registry.lookupExpressionRequired(
								new NodeInstanceDesc.Key(bNodes.RdFlush, endStage, ""))).toList();
						String flushCond = flushConds.get(0);
						if (!flushCond.startsWith("MISSING_") && flushConds.stream().skip(1).anyMatch(otherFlushCond -> !otherFlushCond.startsWith("MISSING_") && !flushConds.get(0).equals(otherFlushCond))) {
							logger.warn("RdFlush conditions don't match up between end stages for ISAX {}: {}", nodeKey.getISAX(), flushConds);
						}
						List<Optional<NodeInstanceDesc>> wrflushOptConds = endStages.stream().map(endStage -> registry.lookupOptionalUnique(
								new NodeInstanceDesc.Key(bNodes.WrFlush, endStage, ""))).toList();
...
//							logger.warn("WrFlush conditions don't match up between end stages for ISAX {}: {}", nodeKey.getISAX(), wrflushOptConds);
//						}
						...
						counterLogic += tab+tab+"$stop;\n";
						counterLogic += tab+"end\n";
						counterLogic += tab+String.format("%s <= (!(%s) ? (%s - (%s ? 1 : 0)) : 0) + (%s ? 1 : 0);\n", counterRegName, flushCond, counterRegName, commitCond, issueCond);
						counterLogic += "end\n";
					...
					ret.outputs.add(new NodeInstanceDesc(
						new NodeInstanceDesc.Key(Purpose.REGULAR, WrStallISAXEntry, nodeKey.getStage(), nodeKey.getISAX(), aux),
						stallCond, ExpressionType.AnyExpression, requestedFor
					));
					ret.outputs.add(new NodeInstanceDesc(
						new NodeInstanceDesc.Key(Purpose.REGULAR, WrStallISAXEntryEarly, nodeKey.getStage(), nodeKey.getISAX(), aux),
						stallCondEarly, ExpressionType.AnyExpression, requestedFor
					));
	...
	private boolean implementRdAnyValid(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
		...
		List<PipelineStage> endStages = spawn_instr_stage.entrySet().stream()
		...
		if (endStageOffenders.size() > 0) {
			logger.warn("SpawnRdIValidStrategy_{}: Encountered sub-pipeline split before [{}], counter may end up invalid",
				nodeKey.toString(),
		...
		if (!instrDesc.GetRunsAsDynamic()) {
			...
			if (endStageOffenders.size() > 0) {
				logger.warn("SpawnRdIValidStrategy_{}: Encountered single-stage sub-pipelines [{}] of statically-scheduled ISAX, which indicates immediate completion",
					nodeKey.toString(),
					endStageOffenders.stream().map(endStage -> endStage.getName()).reduce((a,b)->a+","+b).orElse("")
				);
			}
		...
		out.accept(NodeLogicBuilder.fromFunction("SpawnRdIValidStrategy_"+nodeKey.toString(), registry -> {
			var ret = new NodeLogicBlock();
			String entryCond = String.format("(%s && %s)", 
				registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, parentStage, nodeKey.getISAX())),
			...
			if (instrDesc.GetRunsAsDynamic() && !endStages.isEmpty()) {
				...
				Optional<PipelineStage> isaxAnyValidIntfStage_opt = op_stage_instr.getOrDefault(bNodes.RdAnyValid, new HashMap<>()).entrySet().stream()
					.filter(stage_instrs -> stage_instrs.getValue().contains(nodeKey.getISAX()) && parentFront.isAround(stage_instrs.getKey()) && stage_instrs.getKey().getNext().isEmpty())
					.map(stage_instrs -> stage_instrs.getKey())
					.findAny();
				if (isaxAnyValidIntfStage_opt.isEmpty()) {
					logger.error("SpawnRdIValidStrategy: Cannot find RdAnyValid of ISAX {} below spawn base stage {}", nodeKey.getISAX(), parentStage.getName());
				}
				else {
					dynamicAnyValidCond = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(Purpose.WIREDIN, bNodes.RdAnyValid, isaxAnyValidIntfStage_opt.get(), nodeKey.getISAX()));
				}
				endStagesToWatch = endStages.stream().flatMap(endStage -> endStage.getPrev().stream()).filter(beforeEndStage -> !entryStages.contains(beforeEndStage)).toList();
//				ret.logic += String.format("assign %s = %s || %s;\n",
//					signalName,
//					registry.lookupExpressionRequired(new NodeInstanceDesc.Key(Purpose.WIREDIN, bNodes.RdAnyValid, parentStage, nodeKey.getISAX())),
//					entryCond);
			}
			...
			if (!endStagesToWatch.isEmpty()) {
				...
				long maxCounterElements = endStagesToWatch.stream()
						.flatMap(endStage -> endStage.streamPrev_bfs())
						.count();
					
				int counterWidth = 0; while ((maxCounterElements >> counterWidth) != 0) ++counterWidth;
				...
				Stream<String> endStagesConds = endStagesToWatch.stream().map(endStage -> registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, endStage, nodeKey.getISAX())));
				counterLogic += endStagesConds.map(endCond -> String.format(" - %d'(%s)", counterWidth_, endCond)).reduce((a,b) -> a+b).orElse("");
				...
				ret.outputs.add(new NodeInstanceDesc(
					new NodeInstanceDesc.Key(Purpose.REGULAR, counterNode, nodeKey.getStage(), nodeKey.getISAX()),
					counterRegName,
			...
			ret.logic += String.format("assign %s = %s%s%s;\n",
				signalName,
				entryCond,
				dynamicAnyValidCond.isEmpty() ? "" : (" || " + dynamicAnyValidCond),
				counterCond.isEmpty() ? "" : (" || " + counterCond)
			);

			ret.outputs.add(new NodeInstanceDesc(
				new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.RdAnyValid, nodeKey.getStage(), nodeKey.getISAX()),
				signalName,
				ExpressionType.WireName
			));
	...
	private boolean implementStallISAXEntry(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
		if (!nodeKey.getNode().equals(WrStallISAXEntry) || !nodeKey.getPurpose().matches(Purpose.REGULAR) || nodeKey.getISAX().isEmpty() || nodeKey.getAux() != 0)
			return false;
		...
		out.accept(NodeLogicBuilder.fromFunction("SpawnRdIValidStrategy_" + nodeKey.toString(false), (registry, aux) -> {
			...
			ret.outputs.add(new NodeInstanceDesc(
				new NodeInstanceDesc.Key(WrStallISAXEntry, nodeKey.getStage(), nodeKey.getISAX()),
				"PLACEHOLDER_"+nodeKey.toString(),
			...
			if (!extraStallCond.isEmpty() || !stallSubpipeCond.equals("0") && !stallSubpipeCond.equals("1'b0")) {
				...
				if (!extraStallCond.isEmpty()
					&& !op_stage_instr.getOrDefault(bNodes.RdStall, new HashMap<>()).entrySet().stream()
						.anyMatch(stage_instrs -> stage_instrs.getValue().contains(nodeKey.getISAX())
								&& entryStages.contains(stage_instrs.getKey()))) {
					...
					for (NodeInstanceDesc subNode : registry.lookupAll(new NodeInstanceDesc.Key(WrStallISAXEntryEarly, nodeKey.getStage(), ""), false)) {
						if (subNode.getKey().getISAX().equals(nodeKey.getISAX())) {
							//The sub-node should have a unique identifier.
							assert(subNode.getKey().getAux() != 0);
					...
					for (PipelineStage preStage : nodeKey.getStage().getPrev()) if (preStage.getKind() == StageKind.Core) {
						//Apply the 'early stall ISAX spawn entry' condition to the stages preceding the spawn base stage.
						String earlyIValid = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, preStage, nodeKey.getISAX()));
						...
						ret.outputs.add(new NodeInstanceDesc(
							new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrStall, preStage, "", aux),
							earlyExtraStallCond,
							ExpressionType.AnyExpression
						));
					...
					for (PipelineStage entryStage : entryStages) {
						...
						ret.outputs.add(new NodeInstanceDesc(
							new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrStall, entryStage, "", aux),
							stallCondBaseExpr,
				...
				//Apply the collected stall conditions (WrStallISAXEntry unless done in prior stage, sub-pipeline entry stalls) to the spawn base stage.
				String stallCondWire = nodeKey.toString(false) + "_stallcond_s";
				ret.declarations += String.format("wire %s;\n", stallCondWire);
				ret.logic += String.format("assign %s = %s && (%s || %s) && !(%s);\n", stallCondWire, ivalidIn, stallSubpipeCond, stallCondBaseExpr, flushSubpipeCond);
				ret.outputs.add(new NodeInstanceDesc(
					new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrStall, nodeKey.getStage(), "", aux),
					stallCondWire,
					ExpressionType.WireName
				));
	...
	private boolean implementRdIValid(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
		if (nodeKey.getNode().equals(bNodes.WrDeqInstr) && nodeKey.getISAX().isEmpty() && nodeKey.getAux() == 0) {
			...
			if (implementedWrDeqInstrIValidDeps.add(nodeKey.getStage())) {
				...
				List<Map.Entry<String, PipelineStage>> relevantISAXes = spawn_instr_stage.values().stream()
					.flatMap(instr_stageMap -> instr_stageMap.entrySet().stream())
					.filter(instr_stage -> parentFront.isAround(instr_stage.getValue()))
					.flatMap(instr_stage -> instr_stage.getValue().streamPrev_bfs()
				...
				out.accept(NodeLogicBuilder.fromFunction("SpawnRdIValidStrategy_addDependency_"+nodeKey.toString(false), registry -> {
					var ret = new NodeLogicBlock();
					relevantISAXes.forEach(isaxStage -> registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, isaxStage.getValue(), isaxStage.getKey())));
					return ret;
				}));
			}
			return false;
		...
		if (!(nodeKey.getNode().equals(bNodes.RdIValid))
				|| !nodeKey.getPurpose().matches(Purpose.PIPEDIN) || nodeKey.getAux() != 0)
			return false;
		if (nodeKey.getISAX().isEmpty()) {
			assert(false); //Shouldn't happen.
			return false;
			...
			out.accept(NodeLogicBuilder.fromFunction("SpawnRdIValidStrategy_fromParent_" + nodeKey.toString(false), (registry, aux) -> {
				...
				if (allISAXes.get(nodeKey.getISAX()).GetRunsAsDynamic() && nodeKey.getStage().getNext().isEmpty()) {
					...
					ret.outputs.add(new NodeInstanceDesc(
						...
						"UNDEFINED-dynamicISAX~" + nodeKey.toString(false),
						ExpressionType.AnyExpression
					));
				}
				ret.declarations += String.format("wire %s;\n", signalName);
				//RdIValid must be set in the parent stage.
				String ivalidIn = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, nodeKey.getStage().getParent().get(), nodeKey.getISAX()));
				//The instruction entering the parent stage must be actually valid.
				String instagevalidIn = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdInStageValid, nodeKey.getStage().getParent().get(), ""));
				//The instruction entering the parent stage must be actually allowed to enter.
				// -> commented out for now, as the entry stall condition is either applied to the preceding stage or the base stage and all related sub-pipeline entries.
				//    (by the builder for WrStallISAXEntry)
				Optional<NodeInstanceDesc> isaxentryStallIn_opt = Optional.empty(); //registry.lookupOptional(new NodeInstanceDesc.Key(WrStallISAXEntry, nodeKey.getStage().getParent().get(), nodeKey.getISAX()));
				ret.logic += String.format("assign %s = %s && %s%s;\n", 
					signalName,
					ivalidIn, instagevalidIn, isaxentryStallIn_opt.isPresent() ? (" && !" + isaxentryStallIn_opt.get().getExpressionWithParens()) : ""
				);
				ret.outputs.add(new NodeInstanceDesc(
						new NodeInstanceDesc.Key(Purpose.PIPEDIN, bNodes.RdIValid, nodeKey.getStage(), nodeKey.getISAX()),
						signalName,
						ExpressionType.WireName
				));
				
				//Also set WrDeqInstr if the instruction does enter its sub-pipeline.
				NodeInstanceDesc.Key wrDeqInstrKey = new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrDeqInstr, nodeKey.getStage().getParent().get(), "", aux);
				// Request instantiation of the combined WrDeqInstr, and of the to-Core interface as needed.
				registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.WrDeqInstr, nodeKey.getStage().getParent().get(), ""), requestedFor);
	...
	public void implement(Consumer<NodeLogicBuilder> out, Iterable<NodeInstanceDesc.Key> nodeKeys, boolean isLast) {
		...
		while (nodeKeyIter.hasNext()) {
			...
			if (this.implementRdIValid(out, nodeKey)
					|| this.implementRdAnyValid(out, nodeKey)
					|| this.implementStallISAXEntry(out, nodeKey)
