...
/** Strategy that produces pipe utility signals (stall-, flush- related) and default implementations for per-ISAX spawn validReq,validResp,addr */
public class DecoupledPipeStrategy extends MultiNodeStrategy {
	 ...
	 * @param spawnRDAddrOverrides Custom SCAL<->Core interfaces that specify the destination register address/ID for ISAXes entering a spawn stage.
	 ...
	 */
	public DecoupledPipeStrategy(StrategyBuilders strategyBuilders,
	...
	@Override
	public void setLanguage(Verilog lang) { this.language = lang; }
	...
	private void CreateExtraPipeSignals(NodeLogicBlock logicBlock, NodeRegistryRO registry, PipelineStage onlyStartSpawnStage, PipelineStage spawnStage ) {
		...
		if(startSpawnFront.isBefore(spawnStage, false)) {
			List<PipelineStage> stages = this.core.GetRootStage().getAllChildren()
			...
			for (PipelineStage flStage : stages) {
				...
				if (!flStage.getContinuous()) {
					logger.error("DecoupledPipeStrategy CreateExtraPipeSignals: Encountered non-continuous stage ("
						+ flStage.getName() + "), which is not supported.");
				}
				prevStagePos = flStage.getStagePos();
				String flStageRdStall = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdStall, flStage, ""), commonRequestedFor);
				String flStageWrStall = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.WrStall, flStage, ""));
				stallFrSpawn = flStageWrStall
						+(stallFrSpawn.isEmpty()?"":",")+stallFrSpawn;
				stallShiftReg = flStageRdStall
						+(stallShiftReg.isEmpty()?"":",")+stallShiftReg;
				stallShiftReg = flStageWrStall+" || "+stallShiftReg;
				stallFrCore = flStageRdStall+" || "+flStageWrStall
						+(stallFrCore.isEmpty()?"":",")+stallFrCore;
		...
		logicBlock.outputs.add(new NodeInstanceDesc(
				new NodeInstanceDesc.Key(PseudoNode_StartSpawnToSpawnPipe_Flush, spawnStage, ""), 
				flush, 
		...
		logicBlock.outputs.add(new NodeInstanceDesc(
				new NodeInstanceDesc.Key(PseudoNode_StartSpawnToSpawnPipe_StallShiftReg, spawnStage, ""), 
				stallShiftReg, 
				ExpressionType.AnyExpression,
				stallRequestedFor));
		logicBlock.outputs.add(new NodeInstanceDesc(
				new NodeInstanceDesc.Key(PseudoNode_StartSpawnToSpawnPipe_StallFrSpawn, spawnStage, ""), 
				stallFrSpawn, 
				ExpressionType.AnyExpression,
				stallRequestedFor));
		logicBlock.outputs.add(new NodeInstanceDesc(
				new NodeInstanceDesc.Key(PseudoNode_StartSpawnToSpawnPipe_StallFrCore, spawnStage, ""), 
				stallFrCore, 
				ExpressionType.AnyExpression,
				stallRequestedFor));
		
		String fenceValidExpr = startSpawnFront.asList().stream()
			.map(startSpawnStage -> registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, startSpawnStage, SCAL.PredefInstr.fence.instr.GetName())))
			.reduce((a,b) -> a+" || "+b).orElse("1'b0");
		String killValidExpr = startSpawnFront.asList().stream()
			.map(startSpawnStage -> registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, startSpawnStage, SCAL.PredefInstr.kill.instr.GetName())))
			.reduce((a,b) -> a+" || "+b).orElse("1'b0");
		
		logicBlock.outputs.add(new NodeInstanceDesc(
			new NodeInstanceDesc.Key(PseudoNode_StartSpawnToSpawnPipe_Fence, spawnStage, ""), 
			"("+fenceValidExpr+")",
			ExpressionType.AnyExpression));
		logicBlock.outputs.add(new NodeInstanceDesc(
			new NodeInstanceDesc.Key(PseudoNode_StartSpawnToSpawnPipe_KillAll, spawnStage, ""), 
			"("+killValidExpr+")",
			ExpressionType.AnyExpression));
	}
	...
	private boolean implementStartSpawnToSpawnPipe(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
		...
		if ((node.equals(PseudoNode_StartSpawnToSpawnPipe_Flush)
			|| node.equals(PseudoNode_StartSpawnToSpawnPipe_StallShiftReg)
			|| node.equals(PseudoNode_StartSpawnToSpawnPipe_StallFrSpawn)
			|| node.equals(PseudoNode_StartSpawnToSpawnPipe_StallFrCore)
			|| node.equals(PseudoNode_StartSpawnToSpawnPipe_KillAll)
			|| node.equals(PseudoNode_StartSpawnToSpawnPipe_Fence))) {
	...
	private boolean implementAddrSizeFIFO(Consumer<NodeLogicBuilder> out,
			SCAIEVNode spawnNode, AdjacentNode adj, PipelineStage stage, String isax,
			PipelineStage spawnSubStage) {
		...
		if (isNew) {
			...
			if (relevantStartSpawnStages.size() > 1) {
				logger.warn("DecoupledPipeStrategy - Got several 'start spawn' stages for ISAX '" + isax + "', but can only handle one spawn start at a time");
			...
			List<Optional<CustomCoreInterface>> defaultAddrOverrides = relevantStartSpawnStages.stream()
				.map(startSpawnStage -> {
			...
			var builder = NodeLogicBuilder.fromFunction("DecoupledPipeStrategy_AddrSizeFIFO_"+spawnNodeKey.toString(false), (registry, aux) -> {
				...
				if (fifoBuilderDesc.forAddr && addrNode_opt.get().validBy != AdjacentNode.validReq) {
					fifoBuilderDesc.forAddr = false;
					logger.error("Node " + addrNode_opt.get().name + " is set to sample by " + addrNode_opt.get().validBy.name() + ", expected validReq");
				}
				...
				if (fifoBuilderDesc.forAddr) {
		    		...
		    		if (bNodes.IsUserBNode(spawnNode) && addrNode_opt.isPresent()) {
		    			...
		    			for (int i = 0; i < relevantStartSpawnStages.size(); ++i)
		    				addrReadSigs[i] = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(nonspawnAddrNode, relevantStartSpawnStages.get(i), ""), addrRequestedFor);
			    		...
			    		if (!fromCoreAddrNode_opt.isPresent()) {
			    			...
			    			for (int i = 0; i < relevantStartSpawnStages.size(); ++i) {
					    		addrReadSigs[i] = registry.lookupExpressionRequired(
					    			new NodeInstanceDesc.Key(fromCoreAddrNode_opt.get(), relevantStartSpawnStages.get(i), ""),
					    			addrRequestedFor
					    		);
			    			}
			    			addrW = addrNode_opt.get().size;
		    			...
		    			for (int i = 0; i < relevantStartSpawnStages.size(); ++i) {
		    				...
		    				if (defaultAddrOverrides.get(i).isPresent()) {
		    					...
		    					addrReadSigs[i] = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdInstr, relevantStartSpawnStages.get(i), ""), addrRequestedFor)
		    						+ "[11:7]";
		    		...
		    		if (addrW == 0) {
		    			if (fifoBuilderDesc.forAddr) logger.error("Cannot build default address for " + spawnNode.name);
		    			fifoBuilderDesc.forAddr = false;
		    		}
		    		else {
				...
				if (fifoBuilderDesc.forSize
						&& !spawnNode.equals(bNodes.WrMem_spawn) && !spawnNode.equals(bNodes.RdMem_spawn)) {
					fifoBuilderDesc.forSize = false;
				...
				if (fifoBuilderDesc.forSize && sizeNode_opt.get().validBy != AdjacentNode.validReq) {
					fifoBuilderDesc.forSize = false;
					logger.error("Node " + sizeNode_opt.get().name + " is set to sample by " + sizeNode_opt.get().validBy.name() + ", expected validReq");
				}
				...
				if (fifoBuilderDesc.forSize) {
	    			...
	    			for (int i = 0; i < relevantStartSpawnStages.size(); ++i) {
	    				...
	    				sizeReadSigs[i] = registry.lookupExpressionRequired(
	    					new NodeInstanceDesc.Key(bNodes.RdInstr, relevantStartSpawnStages.get(i), ""),
	    					sizeRequestedFor
	    				) + "[14:12]";
	    			}
    			...
    			String FIFOmoduleName = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(Purpose.HDL_MODULE, DecoupledStandardModulesStrategy.makeFIFONode(), core.GetRootStage(), ""));
    			...
    			for (int i = 0; i < relevantStartSpawnStages.size(); ++i) {
    				...
    				String curStartSpawnValid = "("
						...
						+ " && !" + registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.WrStall, startSpawnStage, ""), commonRequestedFor) //WrStall is not generally included in RdStall
						+ ")";
    				...
    				if (i > 0) {
    					...
    					for (int i_stallcond = i; i_stallcond > 0; --i_stallcond) {
    						fifoWriteStageStallConds[i_stallcond] += (fifoWriteStageStallConds[i_stallcond].endsWith("(")?"":" || ")
    							+ curStartSpawnValid;
    					}
    				}
    			}
    			...
    			String fifoReadCondExpr = registry.lookupExpressionRequired(
    				new NodeInstanceDesc.Key(bNodes.WrCommit_spawn_validReq, stage, isax),
    				commonRequestedFor
    			);
//    			String fifoReadCondExpr = registry.lookupExpressionRequired(
//    				new NodeInstanceDesc.Key(bNodes.GetAdjSCAIEVNode(spawnNode, AdjacentNode.validReq).get(), stage, isax),
//    				commonRequestedFor
//    			);
    			...
    			String clearCond = (stage.getKind() == StageKind.Decoupled)
					? registry.lookupExpressionRequired(new NodeInstanceDesc.Key(DecoupledPipeStrategy.PseudoNode_StartSpawnToSpawnPipe_KillAll, stage, ""), commonRequestedFor)
					: "0";
				ret.declarations += String.format("wire %s;\n", outValidWire);
				ret.declarations += String.format("wire %s;\n", outNotFullWire);
				ret.declarations += String.format("wire [%d-1:0] %s;\n", fifoEntryW, outDataWire);
				ret.logic += "\n"+FIFOmoduleName + " #( "+fifoDepth+", "+fifoEntryW +" ) "
					+ fifoName+"_inst (\n"
    			...
    			if (fifoBuilderDesc.forAddr) {
        			...
        			ret.outputs.add(new NodeInstanceDesc(
    					new NodeInstanceDesc.Key(Purpose.WIREDIN_FALLBACK, addrNode_opt.get(), stage, isax), 
    					addrFromFIFOWire, 
    			...
    			if (fifoBuilderDesc.forSize) {
        			...
        			ret.outputs.add(new NodeInstanceDesc(
    					new NodeInstanceDesc.Key(Purpose.WIREDIN_FALLBACK, sizeNode_opt.get(), stage, isax), 
    					sizeFromFIFOWire, 
    					ExpressionType.WireName,
    					sizeRequestedFor
        			));
    			...
    			for (int i = 0; i < fifoWriteStageStallConds.length; ++i) {
    				...
    				PipelineStage startSpawnStage = relevantStartSpawnStages.get(i);
    				fifoWriteStageStallConds[i] += (fifoWriteStageStallConds[i].isEmpty() ? "" : " || ")
						+ String.format("%s && !%s",
								registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, startSpawnStage, isax), commonRequestedFor),
								outNotFullWire);
        			...
        			ret.outputs.add(new NodeInstanceDesc(
    					new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrStall, relevantStartSpawnStages.get(i), "", aux), 
    					stallCondWire, 
    					ExpressionType.WireName,
    					commonRequestedFor
        			));
	...
	private boolean implementSpawnNodeRequest(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
		if (!(nodeKey.getPurpose().matches(Purpose.WIREDIN_FALLBACK) || nodeKey.getPurpose().matches(Purpose.REGULAR))
			...
			|| nodeKey.getStage().getChildren().isEmpty()
			|| nodeKey.getAux() != 0)
			return false;
		SCAIEVNode spawnNode = bNodes.GetNonAdjNode(nodeKey.getNode());
		assert(spawnNode.isSpawn());
		
		if (spawnNode.name.isEmpty()
			|| !spawn_instr_stage.containsKey(spawnNode)
			|| !spawn_instr_stage.get(spawnNode).containsKey(nodeKey.getISAX())
			|| !new PipelineFront(nodeKey.getStage())
			   .isAround(spawn_instr_stage.get(spawnNode).get(nodeKey.getISAX())))
			return false;
		...
		case validReq:
			...
			settings.triggerable = TriggerableNodeLogicBuilder.fromFunction(builderName, triggerKey, registry -> {
				...
				//      -> The better approach may be to have a 'cancel' node, which would be more in line with what dynamic decoupled would require
				//      This also needs consideration for Register Renaming: For cancellation, need to copy the previous logical register value into the new physical register.
				var wiredinValidreq_opt = registry.lookupOptionalUnique(new NodeInstanceDesc.Key(Purpose.WIREDIN, nodeKey.getNode(), nodeKey.getStage(), nodeKey.getISAX()));
				
				String isValid = registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, spawnSubStage, nodeKey.getISAX()));
				String isStalling = "";
				for (NodeInstanceDesc stallInstance : registry.lookupAll(new NodeInstanceDesc.Key(bNodes.WrStall, spawnSubStage, ""), false, requestedFor)) {
				...
				if (wiredinValidreq_opt.isPresent() && settings.implementRegular) {
					...
					ret.logic += String.format("assign %s = %s && %s;\n", validReqMainWireName, validReqSubpipeWireName, wiredinValidreq_opt.get().getExpression());
					ret.outputs.add(new NodeInstanceDesc(
						new NodeInstanceDesc.Key(Purpose.REGULAR, nodeKey.getNode(), nodeKey.getStage(), nodeKey.getISAX()),
						validReqMainWireName,
						ExpressionType.WireName,
						requestedFor
					));
		...
		case addrReq:
			...
			out.accept(NodeLogicBuilder.fromFunction(builderName, registry -> {
				NodeLogicBlock ret = new NodeLogicBlock();
				if (registry.lookupOptionalUnique(new NodeInstanceDesc.Key(Purpose.WIREDIN, nodeKey.getNode(), nodeKey.getStage(), nodeKey.getISAX())).isPresent()) {
					//If the ISAX already provides this signal, there is no need to generate it.
					return ret;
				}
	...
	public void implement(Consumer<NodeLogicBuilder> out, Iterable<NodeInstanceDesc.Key> nodeKeys, boolean isLast) {
		...
		while (nodeKeyIter.hasNext()) {
			...
			if (implementStartSpawnToSpawnPipe(out, nodeKey)
				|| implementSpawnNodeRequest(out, nodeKey)) {
				nodeKeyIter.remove();
			}
		}
	}
