...
public class DecoupledStandardModulesStrategy extends SingleNodeStrategy {
	...
	private static EnumSet<FIFOFeature> getFeaturesFromFIFONode(SCAIEVNode simplefifoNode) {
		...
		while (!nameSuffix.isEmpty()) {
			...
			for (FIFOFeature feature : FIFOFeature.values()) if (feature != FIFOFeature.NotAFeature) {
	...
	private String FIFOModule(String moduleName, EnumSet<FIFOFeature> features) {
		...
		if (hasWriteFront)
			writeToExpression = String.format("write_front_i ? (%s) : write_pointer",
					String.format("read_valid_i ? read_pointer : prevPointer(read_pointer)"));
		...
		String FIFOModuleContent = "\n"
				...
				+ "    output                not_full,\n"
				+ (hasLevel ?
				  "    output [$clog2(NR_ELEMENTS)-1:0] level_trim,\n":""
				  )
				+ (hasReadahead ?
				  "    input                 readahead_i,\n":""
				  )
				...
				+ "            && val == FIFOPointer_t'(NR_ELEMENTS-1))\n"
				+ "        return 0; // explicit wrap if NR_ELEMENTS is not a power of 2\n"
				+ "    return val + FIFOPointer_t'(1);\n"
				+ "endfunction\n"
				+ (needsPrevPointer ?
				  "function FIFOPointer_t prevPointer(input FIFOPointer_t val);\n"
				+ "    if ($clog2(NR_ELEMENTS) == $clog2(NR_ELEMENTS+1)\n"
				+ "            && val == FIFOPointer_t'(0))\n"
				+ "        return FIFOPointer_t'(NR_ELEMENTS-1); // explicit wrap if NR_ELEMENTS is not a power of 2\n"
				+ "    return val - FIFOPointer_t'(1);\n"
				+ "endfunction\n" : ""
				  )
				+ "\n"
				...
				+ "        "
				  + (hasWriteFront
					? "if(!(write_valid_i && write_front_i))" //increment due to read, decrement due to write (+-0)
					: "")
...
//		if(startSpawnStagesList.stream().anyMatch(startSpawnStage -> 
...
//			returnStr += "`define LATER_FLUSHES\n";
		returnStr += "module "+ShiftmoduleName+(laterFlushes?"_LateFlush":"")+" #(\n"
			...
			+ "   if(("+(laterFlushes?"flush_i[1] && ":"")+"stall_i[0]) || ( stall_i[0] && !stall_i[1])) //  (flush_i[0] && !stall_i[0]) not needed, data_i should be zero in case of flush for shift valid bits in case of flush\n"
			+ "        shift_reg[1] <= 0 ;\n"
			+ "   for(int i=2;i<=NR_ELEMENTS;i = i+1) begin\n"
			+ "        if((i+START_STAGE)<=WB_STAGE) begin\n"
			+ "          if(("+(laterFlushes?"flush_i[i] && ":"")+"stall_i[i-1]) || ("+(laterFlushes?"flush_i[i-1] && ":"")+"!stall_i[i-1]) || ( stall_i[i-1] && !stall_i[i]))\n"
			+ "            shift_reg[i] <=0;\n"
			+ "          else if(!stall_i[i-1])\n"
	...
	public Optional<NodeLogicBuilder> implement(Key nodeKey) {
		...
		else if (nodeKey.getNode().name.equals("SimpleShift") || (nodeKey.getNode().name.equals("SimpleShift_LateFlush") && (withLateFlush=true))) {
