...
public class SpawnFenceStrategy extends SingleNodeStrategy {
	...
	public Optional<NodeLogicBuilder> implement(Key nodeKey) {
    	...
    	if (nodeKey.getPurpose().matches(Purpose.REGULAR)
    			&& nodeKey.getNode().equals(disaxfence_stall_node)
    		...
    		List<PipelineStage> preSpawnStages = startSpawnStages.streamNext_bfs(intermediateStage -> !spawnStages.contains(intermediateStage))
				...
				.toList();
    		List<String> relevantISAXes = op_stage_instr.entrySet().stream()
    			.filter(op_stage_instr_entry -> {
	    			...
	    			return op.isSpawn() && !op.isAdj()
		    			...
		    			//TODO: Condition whether data hazard handling is enabled for custom registers (in case there will be a toggle for that).
    		...
    		return Optional.of(NodeLogicBuilder.fromFunction("disaxfence_stall", (registry,aux) -> {
    			...
    			String fenceActiveCondition = startSpawnStages.asList().stream()
        				.map(startSpawnStage ->
        					registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, startSpawnStage, SCAL.PredefInstr.fence.instr.GetName()), fenceRequestedFor)
        				).reduce((a,b) -> a+" || "+b).orElse("1'b0");
    			String intermediateCondition = relevantISAXes.stream()
    				.flatMap(isax -> preSpawnStages.stream().map(stage -> Map.entry(stage, isax))) //Get all Intermediate Stage-ISAX combinations
    				.map(entry -> registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, entry.getKey(), entry.getValue()), fenceOnlyRequestedFor))
    				.reduce((a,b) -> a+" || "+b).orElse("1'b0");
    			String spawnActiveCondition = relevantISAXes.stream()
    				.flatMap(isax -> spawnStages.asList().stream().map(stage -> Map.entry(stage, isax))) //Get all Spawn Stage-ISAX combinations
    				.map(entry -> registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdAnyValid, entry.getKey(), entry.getValue()), fenceOnlyRequestedFor))
    				.reduce((a,b) -> a+" || "+b).orElse("1'b0");
    			...
    			ret.outputs.add(new NodeInstanceDesc(
    				new NodeInstanceDesc.Key(disaxfence_stall_node, core.GetRootStage(), ""),
    				wireName, ExpressionType.WireName,
    				fenceOnlyRequestedFor
    			));
    			startSpawnStages.asList().forEach(startSpawnStage -> 
    				ret.outputs.add(new NodeInstanceDesc(
    					new NodeInstanceDesc.Key(Purpose.REGULAR, bNodes.WrStall, startSpawnStage, "", aux),
    					wireName, ExpressionType.AnyExpression,
    					fenceOnlyRequestedFor
    				))
    			);
    			return ret;
    			...
    			&& nodeKey.getNode().equals(bNodes.RdFence)
    			&& allISAXes.containsKey(SCAL.PredefInstr.fence.instr.GetName())
    		...
    		return Optional.of(NodeLogicBuilder.fromFunction("RdFence_"+nodeKey.getStage().getName(), (registry,aux) -> {
    			...
    			ret.outputs.add(new NodeInstanceDesc(
    				new NodeInstanceDesc.Key(bNodes.RdFence, nodeKey.getStage(), nodeKey.getISAX()),
    				fenceActiveCondition,
