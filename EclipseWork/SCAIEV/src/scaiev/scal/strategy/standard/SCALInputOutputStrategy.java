...
public class SCALInputOutputStrategy extends SingleNodeStrategy {
	 ...
	 */
	public SCALInputOutputStrategy(Verilog language, BNode bNodes) {
	...
	public Optional<NodeLogicBuilder> implement(Key nodeKey) {
		...
		boolean isInputPort = nodeKey.getPurpose().equals(Purpose.MARKER_FROMCORE_PIN) || nodeKey.getPurpose().equals(Purpose.MARKER_FROMISAX_PIN);

		if (isOutputPort || isInputPort) { // should be a MODULEOUTPUT node
			SCAIEVNode outnode_correct_polarity = (isInputPort == nodeKey.getNode().isInput) ? nodeKey.getNode() : nodeKey.getNode().NodeNegInput();
			Key outNodeKey = new Key(
				NodeInstanceDesc.Purpose.WIREDIN,
				outnode_correct_polarity,
				nodeKey.getStage(),
				nodeKey.getISAX(),
				0);
			...
			return Optional.of(NodeLogicBuilder.fromFunction("SCALInputOutputStrategy ("+nodeKey.toString()+")", registry -> {
				...
				if(registry.lookupOptional(nodeKey).isEmpty()) {
					...
					boolean isISAX = nodeKey.getPurpose().equals(Purpose.MARKER_TOISAX_PIN) || nodeKey.getPurpose().equals(Purpose.MARKER_FROMISAX_PIN);
					...
					String newinterf = language.CreateTextInterface(pinNamePrefix + nodeKey.getNode().name, nodeKey.getStage(), guardedISAX, isInputPort, nodeKey.getNode().size, "");
					String pinName = pinNamePrefix
							+ language.CreateFamNodeName(outnode_correct_polarity, nodeKey.getStage(), guardedISAX, false);
										// create a pin datatype to describe the iface pin
					NodeLogicBlock.InterfacePin pin = new NodeLogicBlock.InterfacePin(

						newinterf,
						new NodeInstanceDesc(outNodeKey, pinName, isOutputPort ? ExpressionType.ModuleOutput : ExpressionType.ModuleInput, pinRequestedFor));
					...
					var markerNodeInst = new NodeInstanceDesc(
						new NodeInstanceDesc.Key(nodeKey.getPurpose(), outnode_correct_polarity, nodeKey.getStage(), nodeKey.getISAX(), nodeKey.getAux()),
						"1",
