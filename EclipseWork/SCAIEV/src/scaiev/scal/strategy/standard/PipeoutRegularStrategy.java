...
public class PipeoutRegularStrategy extends SingleNodeStrategy {
	...
	public Optional<NodeLogicBuilder> implement(Key nodeKey) {
		if (nodeKey.getPurpose().matches(NodeInstanceDesc.Purpose.PIPEOUT)) {
			return Optional.of(NodeLogicBuilder.fromFunction("PipeoutRegularStrategy ("+nodeKey.toString(false)+")", (NodeRegistryRO registry) -> {
                var fromKey = new NodeInstanceDesc.Key(NodeInstanceDesc.Purpose.match_REGULAR_WIREDIN_OR_PIPEDIN, nodeKey.getNode(), nodeKey.getStage(), nodeKey.getISAX(), nodeKey.getAux());
