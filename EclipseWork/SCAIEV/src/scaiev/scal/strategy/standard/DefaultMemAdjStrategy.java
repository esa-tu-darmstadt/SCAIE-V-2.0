...
public class DefaultMemAdjStrategy extends MultiNodeStrategy {
	...
	public DefaultMemAdjStrategy(StrategyBuilders strategyBuilders, Verilog language, BNode bNodes, Core core) {
		...
		PipelineFront minDecodePipetoFront = new PipelineFront(minDecodeFront.asList().stream().flatMap(minDecodeStage -> minDecodeStage.getNext().stream()));
		
		this.pipelinedMemSizeStrategy = strategyBuilders.buildNodeRegPipelineStrategy(language, bNodes, minDecodePipetoFront, false, false, false,
			key -> key.getPurpose().matches(Purpose.PIPEDIN)
				&& (key.getNode().equals(bNodes.RdMem_size) || key.getNode().equals(bNodes.WrMem_size)
					)
				&& key.getISAX().isEmpty()
				&& key.getStage().getKind() == StageKind.Core, 
			key->false,
			new DefaultMemSizeStrategy());
		...
		this.spawnPipelinedMemAddrStrategy = makePipelinedMemAddrStrategy(bNodes.RdMem_spawn_defaultAddr, bNodes.WrMem_spawn_defaultAddr).orElse(null);
	}
	...
	protected Optional<MultiNodeStrategy> makePipelinedMemAddrStrategy(SCAIEVNode readDefaultAddrNode, SCAIEVNode writeDefaultAddrNode) {
		...
		PipelineFront minAddrPipetoFront = new PipelineFront(minRdRS1Front.asList().stream().flatMap(minRdRS1Stage -> minRdRS1Stage.getNext().stream()));
		...
		if (core.GetNodes().containsKey(readDefaultAddrNode)) {
			...
			if (!Optional.ofNullable(core.GetNodes().get(writeDefaultAddrNode))
					.map(wrDefaultAddrCoreNode -> wrDefaultAddrCoreNode.GetEarliest().equals(defaultAddrCoreNode.GetEarliest())
						&& wrDefaultAddrCoreNode.GetLatest().equals(defaultAddrCoreNode.GetLatest())).orElse(false)) {
				...
				minAddrPipetoFront = new PipelineFront(minDefaultAddrFront.asList().stream().flatMap(minAddrStage -> minAddrStage.getNext().stream()));
		...
		if (!minAddrPipetoFront.asList().isEmpty()) {
			return Optional.of(strategyBuilders.buildNodeRegPipelineStrategy(language, bNodes, minAddrPipetoFront, false, false, false,
				key -> key.getPurpose().matches(Purpose.PIPEDIN)
					&& (key.getNode().equals(readDefaultAddrNode) || key.getNode().equals(writeDefaultAddrNode))
					&& key.getISAX().isEmpty(),
				key -> false,
				coreProvidesDefaultMemAddr
				? new RequestMemAddrFromCoreStrategy(readDefaultAddrNode, writeDefaultAddrNode, latestDefaultAddrFront)
	...
	protected class DefaultMemSizeStrategy extends SingleNodeStrategy {
		...
		public Optional<NodeLogicBuilder> implement(NodeInstanceDesc.Key nodeKey) {
			if (!(nodeKey.getNode().equals(bNodes.RdMem_size) || nodeKey.getNode().equals(bNodes.WrMem_size))
	...
	public void implement(Consumer<NodeLogicBuilder> out, Iterable<NodeInstanceDesc.Key> nodeKeys, boolean isLast) {
		...
		while (nodeKeyIter.hasNext()) {
			...
			if (nodeKey.getPurpose().matches(Purpose.WIREDIN)
					&& (nodeKey.getNode().equals(bNodes.RdMem_addr) || nodeKey.getNode().equals(bNodes.WrMem_addr))
					&& nodeKey.getISAX().isEmpty()) {
				var requestedFor = new RequestedForSet(nodeKey.getISAX());
