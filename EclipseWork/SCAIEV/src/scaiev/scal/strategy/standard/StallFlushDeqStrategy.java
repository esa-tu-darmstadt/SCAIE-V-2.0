	...
	public Optional<NodeLogicBuilder> implement(NodeInstanceDesc.Key nodeKey) {
		...
		if ((nodeKey.getNode().equals(bNodes.WrStall) || nodeKey.getNode().equals(bNodes.WrFlush)
			...
			return Optional.of(NodeLogicBuilder.fromFunction("StallFlushDeqStrategy ("+nodeKey.toString()+")", (NodeRegistryRO registry, Integer aux) -> {
				...
				for (NodeInstanceDesc subNode : registry.lookupAll(new NodeInstanceDesc.Key(nodeKey.getNode(), nodeKey.getStage(), ""), false)) {
				...
				var output = new NodeInstanceDesc(NodeInstanceDesc.Key.keyWithPurpose(nodeKey, Purpose.REGULAR), nodeWire, ExpressionType.WireName);
		...
		if (nodeKey.getNode().equals(bNodes.RdStallLegacy)
				&& nodeKey.getPurpose().matches(NodeInstanceDesc.Purpose.REGULAR)
				&& nodeKey.getISAX().isEmpty()
				&& nodeKey.getAux() == 0
				&& core.TranslateStageScheduleNumber(core.GetNodes().get(nodeKey.getNode()).GetEarliest()).isAroundOrBefore(nodeKey.getStage(),false)) {
			...
			NodeInstanceDesc.Key generalKey = new NodeInstanceDesc.Key(bNodes.RdStall, nodeKey.getStage(), ""); //RdStall (e.g. from core) without any added conditions
			...
			return Optional.of(NodeLogicBuilder.fromFunction("StallFlushDeqStrategy ("+nodeKey.toString()+")", (NodeRegistryRO registry, Integer aux) -> {
				...
				ret.logic += "assign " + nodeWire + " = " + newValue + ";";
				ret.outputs.add(new NodeInstanceDesc(NodeInstanceDesc.Key.keyWithPurpose(nodeKey, Purpose.REGULAR), nodeWire, ExpressionType.WireName, requestedFor));
				return ret;
			}));
		}
		if ((nodeKey.getNode().equals(bNodes.RdStall) || nodeKey.getNode().equals(bNodes.RdFlush))
			...
			return Optional.of(NodeLogicBuilder.fromFunction("StallFlushDeqStrategy ("+nodeKey.toString()+")", (NodeRegistryRO registry, Integer aux) -> {
				...
				if (nodeKey.getStage().getKind() == StageKind.Sub && nodeKey.getNode().equals(bNodes.RdStall)) {
					...
					newValue += (newValue.isEmpty() ? "" : " || ") + "!" + registry.lookupExpressionRequired(new NodeInstanceDesc.Key(bNodes.RdIValid, nodeKey.getStage(), nodeKey.getISAX()));
				}
				...
				newValue += (newValue.isEmpty() ? "" : " || ") + registry.lookupRequired(generalKey, interfBuilder.requestedFor).getExpressionWithParens();
