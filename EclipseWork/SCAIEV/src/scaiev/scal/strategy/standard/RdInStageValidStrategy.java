...
/** Generates either default implementations or core interface pins for RdInStageValid, and aggregates WIREDIN_FALLBACK or REGULAR subconditions aux nodes by '&&'. */
public class RdInStageValidStrategy extends MultiNodeStrategy {
	...
	private boolean implementSingle_stallmode(Consumer<NodeLogicBuilder> out, NodeInstanceDesc.Key nodeKey) {
		...
		boolean coreSupportsNodeInStage = isCoreStage && Optional.ofNullable(core.GetNodes().get(node))
		...
		if (!coreSupportsNodeInStage && nodeKey.getPurpose().matches(Purpose.WIREDIN_FALLBACK)) {
			if (implementedKeys.add(outFallbackNodeKey)) {
				if (isCoreStage) {
					logger.info("RdInStageValidStrategy: Using !RdStall as a baseline 'instruction valid' condition for stage " + nodeKey.getStage().getName()
							+ ", since the core does not provide its own. This might lead to combinational loops depending on the ISAX/core combination.");
				}
				...
				out.accept(NodeLogicBuilder.fromFunction("RdInStageValidStrategy_"+outFallbackNodeKey.toString(), registry -> {
					...
					String stallFromCore = registry.lookupRequired(new NodeInstanceDesc.Key(bNodes.RdStall, stage, ""), requestedFor).getExpressionWithParens();
					String aggregatedSubconditions = aggregateSubconditions(registry, Purpose.WIREDIN_FALLBACK, stage, requestedFor);
					if (!aggregatedSubconditions.isEmpty()) aggregatedSubconditions = " && " + aggregatedSubconditions;
		...
		if (nodeKey.getPurpose().matches(Purpose.REGULAR)) {
			if (implementedKeys.add(outRegularNodeKey)) {
				...
				out.accept(NodeLogicBuilder.fromFunction("RdInStageValidStrategy_"+outRegularNodeKey.toString(), registry -> {
					...
					var wiredinInst = registry.lookupRequired(new NodeInstanceDesc.Key(Purpose.match_WIREDIN_OR_PIPEDIN, bNodes.RdInStageValid, stage, ""), requestedFor);
