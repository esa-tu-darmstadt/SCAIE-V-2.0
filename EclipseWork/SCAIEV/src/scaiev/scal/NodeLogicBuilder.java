...
public abstract class NodeLogicBuilder {
	...
	/**
	 * Builder procedure that, given a set of existing nodes through the registry parameter, produces logic that outputs the given and possibly more nodes.
	 *  Must support being called several times; the list of NodeInstanceDesc outputs should generally remain stable across calls, regardless of existing nodes.
	 ...
	 * Convenience method that creates a NodeLogicBuilder for a lambda expression
	 * @param name name for toString
	 * @param fn with args (NodeRegistryRO registry), implements {@link NodeLogicBuilder#apply(NodeRegistryRO, int)} ignoring the aux parameter
	 * @return 
	 */
	 ...
	 */
	public static NodeLogicBuilder makeEmpty() {
		return fromFunction("(empty builder)", registry -> new NodeLogicBlock());
	}
}
