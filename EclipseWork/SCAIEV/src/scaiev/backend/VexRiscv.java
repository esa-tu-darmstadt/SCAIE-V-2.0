...
public class VexRiscv extends CoreBackend{
	...
	public void Prepare (HashMap <String,SCAIEVInstr> ISAXes, HashMap<SCAIEVNode, HashMap<PipelineStage,HashSet<String>>> op_stage_instr, Core core, SCALBackendAPI scalAPI, BNode user_BNode) {
		...
		core.PutNode(BNode.RdInStageValid, new CoreNode(1, 0, vexInterfaceStageNames.length, vexInterfaceStageNames.length+1, BNode.RdInStageValid.name));
	}
	...
	private void IntegrateISAX_Build(PipelineStage stage) {
		...
		for(SCAIEVNode operation : op_stage_instr.keySet()) if (!operation.isAdj()) {
			boolean spawnReq = ( (operation.equals(BNode.WrMem_spawn) || operation.equals(BNode.RdMem_spawn) || operation.equals(BNode.WrRD_spawn))  && stage.getStagePos()==this.vex_core.maxStage) ;
	...
	private void IntegrateISAX_BuildIOs(PipelineStage stage) {
		...
		for (SCAIEVNode operation: op_stage_instr.keySet())	if (!operation.isAdj()) {
			...
			boolean spawnValid = !operation.nameParentNode.isEmpty() && FNode.HasSCAIEVNode(operation.nameParentNode) && operation.isSpawn()  && stageNum == this.spawnStages.get(operation);
			...
			if(spawnValid || ContainsOpInStage(operation,stage) || operation.equals(BNode.WrFlush)) {
				...
				for(AdjacentNode adjacent : BNode.GetAdj(operation)) {
					if(adjacent != AdjacentNode.none) {
						if(spawnValid && !operation.nameCousinNode.isEmpty() && this.op_stage_instr.containsKey(BNode.GetSCAIEVNode(operation.nameCousinNode)) && operation.isInput)
							continue;
		...
		if((this.op_stage_instr.containsKey(BNode.RdMem) | this.op_stage_instr.containsKey(BNode.WrMem)) && stageNum == this.vex_core.GetNodes().get(BNode.RdMem).GetEarliest().asInt() ) {
			for(int earlierStage = this.vex_core.GetNodes().get(BNode.RdInstr).GetEarliest().asInt();earlierStage< this.vex_core.GetNodes().get(BNode.RdMem).GetEarliest().asInt(); earlierStage++ ) {
				if(this.op_stage_instr.containsKey(BNode.RdMem))
					interfaces += language.CreateInterface(BNode.GetAdjSCAIEVNode(BNode.RdMem, AdjacentNode.validReq).get(),stages[earlierStage], "");
				if(this.op_stage_instr.containsKey(BNode.WrMem))
					interfaces += language.CreateInterface(BNode.GetAdjSCAIEVNode(BNode.WrMem, AdjacentNode.validReq).get(),stages[earlierStage], "");
			}
	...
	public void IntegrateISAX_BuilPCNodes() {
		...
		if(this.ContainsOpInStage(BNode.WrPC,0)) {
			declareIO += toFile.tab.repeat(2) +language.CreateInterface(BNode.WrPC, stages[0], "")
			+toFile.tab.repeat(2) +language.CreateInterface(BNode.WrPC_valid, stages[0], "");
		...
		if(this.op_stage_instr.containsKey(BNode.WrPC_spawn)) {
			declareIO += toFile.tab.repeat(2) + language.CreateInterface(BNode.WrPC_spawn, stages[spawnStage], "")
			+ toFile.tab.repeat(2) + language.CreateInterface(BNode.WrPC_spawn_valid, stages[spawnStage], "");
			logic +=  toFile.tab.repeat(1)+"jumpInFetch.target_PC := io."+language.CreateNodeName(BNode.WrPC_spawn,stages[spawnStage], "")+";\n"
					+ toFile.tab.repeat(1)+"jumpInFetch.update_PC := io."+language.CreateNodeName(BNode.WrPC_spawn_valid, stages[spawnStage], "")+";\n";
		}
	...
	public void IntegrateISAX_BuildBody(PipelineStage stage) {
		...
		// Simple assigns
		for(SCAIEVNode operation : op_stage_instr.keySet()) if (!operation.isAdj()) {
			if( (stage.getPrev().size()>0) && op_stage_instr.get(operation).containsKey(stage) && (!operation.equals(BNode.WrRD) && !operation.equals(BNode.WrMem) && !operation.equals(BNode.RdMem)) ) { // it cannot be spawn, as stage does not go to max_stage + 1
				if(!FNode.HasSCAIEVNode(operation.name) || stage.getStagePos()>=this.vex_core.GetNodes().get(operation).GetEarliest().asInt()) {
		...
		if(ContainsOpInStage(BNode.WrPC,stage))  {
			thisStagebuild += "jumpInterface_"+stage.getStagePos()+".valid := io."+language.CreateNodeName(BNode.WrPC_valid, stage, "")+" && !arbitration.isStuckByOthers\n";
		...
		if(stage.getStagePos() == this.vex_core.GetNodes().get(BNode.RdMem).GetEarliest().asInt() && (op_stage_instr.containsKey(BNode.WrMem) || op_stage_instr.containsKey(BNode.WrMem_spawn) || op_stage_instr.containsKey(BNode.RdMem) || op_stage_instr.containsKey(BNode.RdMem_spawn)))
	...
	public void IntegrateISAX_MemBuildBody(PipelineStage stage) {
		...
		if(stage.getStagePos() == memStageNum && ((op_stage_instr.containsKey(BNode.WrMem) || op_stage_instr.containsKey(BNode.WrMem_spawn) || op_stage_instr.containsKey(BNode.RdMem) || op_stage_instr.containsKey(BNode.RdMem_spawn)))) {
			String response = "";
			String rdData = "";
			...
			String spawnaddr = "";
			if(rdaddr)
				rdaddrText += "			               when(io."+language.CreateNodeName(BNode.RdMem_addr_valid, stages[memStageNum], "")+") { // set to 0 if no ISAX needs it and removed by synth tool \n"
						+ "			                  dBusAccess.cmd.address := io."+language.CreateNodeName(BNode.RdMem_addr, stages[memStageNum], "")+"\n"
						+ "			               }\n";
			if(wraddr)
				wraddrText += "			               when(io."+language.CreateNodeName(BNode.WrMem_addr_valid, stages[memStageNum], "")+") { // set to 0 if no ISAX needs it and removed by synth tool \n"
						+ "			                  dBusAccess.cmd.address := io."+language.CreateNodeName(BNode.WrMem_addr, stages[memStageNum], "")+"\n"
						+ "			               }\n";
			if(op_stage_instr.containsKey(BNode.WrMem_spawn) || op_stage_instr.containsKey(BNode.RdMem_spawn)) 
				spawnaddr +=  "			               when(io."+language.CreateNodeName(BNode.RdMem_spawn_validReq, stages[spawnStageNum], "")+") {\n"
						+ "			                  dBusAccess.cmd.address := io."+language.CreateNodeName(BNode.RdMem_spawn_addr, stages[spawnStageNum], "")+"\n"
						+ "			               }\n"; 
			...
			if(op_stage_instr.containsKey(BNode.WrMem_spawn)) 
				writeText = language.OpIfNEmpty(writeText, " || ") +  "io."+language.CreateNodeName(BNode.WrMem_spawn_write, stages[spawnStageNum], "");
			...
			if(op_stage_instr.containsKey(BNode.WrMem) || op_stage_instr.containsKey(BNode.WrMem_spawn)) {
				...
				if(op_stage_instr.containsKey(BNode.WrMem))
					writedata +=  "                        when(io."+language.CreateNodeName(BNode.WrMem_validReq, stages[memStageNum], "")+") {\n"
							+ "                            dBusAccess.cmd.data := io."+language.CreateNodeName(BNode.WrMem, stages[memStageNum], "")+"\n"
							+ "                        }\n"; 
				if(op_stage_instr.containsKey(BNode.WrMem_spawn))
					writedata +=  "                        when(io."+language.CreateNodeName(BNode.WrMem_spawn_validReq, stages[spawnStageNum], "")+") {\n"
							+ "                            dBusAccess.cmd.data := io."+language.CreateNodeName(BNode.WrMem_spawn, stages[spawnStageNum], "")+"\n"
							+ "                        }\n"; 
			} else 
				writedata += "                        dBusAccess.cmd.data   := 0 ;\n";
			...
			if(op_stage_instr.containsKey(BNode.RdMem)) {
				invalidTransfer = language.OpIfNEmpty(invalidTransfer  , " || ") +  "io."+ language.CreateNodeName(BNode.RdMem_validReq,stages[memStageNum], "");
			}	 
			if(op_stage_instr.containsKey(BNode.RdMem_spawn) || op_stage_instr.containsKey(BNode.WrMem_spawn)) {
				invalidTransfer = language.OpIfNEmpty(invalidTransfer  , " || ") + "io."+ language.CreateNodeName(BNode.RdMem_spawn_validReq,stages[spawnStageNum], "");
				defaultSigs += "io."+language.CreateNodeName(BNode.RdMem_spawn_validResp, stages[spawnStageNum], "")+" := "+this.language.GetDict(DictWords.False)+";  \n";
			}
			...
			if(op_stage_instr.containsKey(BNode.RdMem_spawn) || op_stage_instr.containsKey(BNode.WrMem_spawn))
				transferSize += "                        when(io."+language.CreateNodeName(BNode.RdMem_spawn_validReq,stages[spawnStageNum], "")+") { dBusAccess.cmd.size    := 2 }\n";
			...
			String rdState = "";
			if(op_stage_instr.containsKey(BNode.WrMem)) {
				nextstateWr += "when (io."+language.CreateNodeName(BNode.WrMem_validReq, stages[memStageNum], "")+") {\n    state := State.IDLE\n}";
				nextstateWr = language.AlignText(language.tab.repeat(7), nextstateWr);
				nextstateWr += "\n";
			}
			if(op_stage_instr.containsKey(BNode.WrMem_spawn)) {
				nextstateWr += "when (io."+language.CreateNodeName(BNode.WrMem_spawn_validReq, stages[spawnStageNum], "")+" && io."+language.CreateNodeName(BNode.WrMem_spawn_write, stages[spawnStageNum], "")+") {\n    state := State.IDLE\nio."+language.CreateNodeName(BNode.WrMem_spawn_validResp, stages[spawnStageNum], "")+" := True;\n}";
				nextstateWr = language.AlignText(language.tab.repeat(7), nextstateWr);
				nextstateWr += "\n";
			}
			if(op_stage_instr.containsKey(BNode.RdMem) | op_stage_instr.containsKey(BNode.RdMem_spawn)) {
				...
				if(op_stage_instr.containsKey(BNode.RdMem)) {
					...
					logicInState +="                        io."+language.CreateNodeName(BNode.RdMem, stages[memStageNum], "")+" := dBusAccess.rsp.data;  \n";
				}
				if(op_stage_instr.containsKey(BNode.RdMem_spawn)) {
					condNextState  = this.language.OpIfNEmpty(condNextState, " || ") +"io."+ language.CreateNodeName(BNode.RdMem_spawn_validReq, stages[spawnStageNum], "") + " && !io."+ language.CreateNodeName(BNode.RdMem_spawn_write, stages[spawnStageNum], "");
					logicInState +="                        io."+language.CreateNodeName(BNode.RdMem_spawn, stages[spawnStageNum], "")+" := dBusAccess.rsp.data;  \n";
					logicInState +="                        when(io."+language.CreateNodeName(BNode.RdMem_spawn_validReq, stages[spawnStageNum], "")+") {\n "
							+"                            io."+language.CreateNodeName(BNode.RdMem_spawn_validResp, stages[spawnStageNum], "")+" := "+this.language.GetDict(DictWords.True)+";  \n"
							+"                        }";
				}
			...
			if(op_stage_instr.containsKey(BNode.WrMem_spawn) || op_stage_instr.containsKey(BNode.RdMem_spawn)) {
				logicText += "io."+language.CreateFamNodeName(BNode.GetAdjSCAIEVNode(BNode.WrMem_spawn, AdjacentNode.defaultAddr).get(), stages[spawnStageNum], "",true)  + " := execute.input(SRC_ADD).asUInt ; \n ";
			}
	...
	public void IntegrateISAX_WrRDBuild(PipelineStage stage) {
		...
		if(ContainsOpInStage(BNode.WrRD,stage) ) {
			...
			toFile.nrTabs++;	 
			toFile.UpdateContent(filePlugin,vexInterfaceStageNames[stage.getStagePos()]+".output(REGFILE_WRITE_DATA) := io."+language.CreateNodeName(BNode.WrRD, stage, "")+";\n");				 		
			toFile.nrTabs--;
			toFile.UpdateContent(filePlugin,"}\n");
		...
		// SPAWN
		int spawnStageNum = this.vex_core.maxStage+1;
		if(op_stage_instr.containsKey(BNode.WrRD_spawn) && stage.getStagePos()==this.vex_core.GetNodes().get(BNode.WrRD).GetLatest().asInt()) {			 
			String logic = "";
			logic += "when (io."+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[spawnStageNum], "")+") {\n "
					+ toFile.tab +vexInterfaceStageNames[stage.getStagePos()]+".output(INSTRUCTION) := ((11 downto 7) ->io."+language.CreateNodeName(BNode.WrRD_spawn_addr, stages[spawnStageNum], "")+", default -> false)\n"
					+ toFile.tab +vexInterfaceStageNames[stage.getStagePos()]+".arbitration.isRegFSpawn := True\n"
					+ toFile.tab +vexInterfaceStageNames[stage.getStagePos()]+".output(REGFILE_WRITE_DATA) := io."+language.CreateNodeName(BNode.WrRD_spawn, stages[spawnStageNum], "")+"\n"
					+ toFile.tab + "}\n"
	...
	private void IntegrateISAX_UpdateConfigFile() {
		...
		if(this.vex_core.GetNodes().get(BNode.RdRS1).GetLatest().asInt()==3) //Disable MulPlugin for 4-stage Vex (cannot build Vex otherwise)
	...
	private boolean ContainsOpInStage(SCAIEVNode operation, int stage) {
		return this.ContainsOpInStage(operation, stages[stage]);
	}
	...
	private void ConfigVex() {
		...
		for (int stageNum = 1; stageNum < vexInterfaceStageNames.length; ++stageNum) {
			...
			this.PutNode( "Bool", stageName+".arbitration.isStuck || (!"+stageName+".arbitration.isValid)", stageName, BNode.RdStall, stages[stageNum]);
		 	this.PutNode( "Bool", stageName+".arbitration.isValid", stageName, BNode.RdInStageValid, stages[stageNum]);
			this.PutNode( "Bool", stageName+".arbitration.isFlushed", stageName, BNode.RdFlush, stages[stageNum]); //|| (!"+vexInterfaceStageNames[stage]+".arbitration.isValid)"
			...
			if(this.vex_core.GetNodes().get(BNode.WrRD).GetLatest().asInt()>=stageNum && this.vex_core.GetNodes().get(BNode.WrRD).GetEarliest().asInt()<=stageNum) {
		...
		for (int stageNum = 1; stageNum < vexInterfaceStageNames.length; ++stageNum) {
			if(stageNum>1) {
				this.PutNode( "Bits", vexInterfaceStageNames[stageNum]+".input(RS1)", vexInterfaceStageNames[stageNum], BNode.RdRS1, stages[stageNum]);
				this.PutNode( "Bits", vexInterfaceStageNames[stageNum]+".input(RS2)", vexInterfaceStageNames[stageNum], BNode.RdRS2, stages[stageNum]);
			}
			if(stageNum>0) {
		...
		this.PutNode( "Bits", vexInterfaceStageNames[stageWrRD]+".output(REGFILE_WRITE_DATA)", vexInterfaceStageNames[stageWrRD], BNode.WrRD_spawn, stages[spawnStage]);
		this.PutNode( "Bool", vexInterfaceStageNames[stageWrRD]+".output(REGFILE_WRITE_VALID)", vexInterfaceStageNames[stageWrRD], BNode.WrRD_spawn_valid, stages[spawnStage]);
		this.PutNode( "Bits", vexInterfaceStageNames[stageWrRD]+".output(INSTRUCTION)", vexInterfaceStageNames[stageWrRD], BNode.WrRD_spawn_addr, stages[spawnStage]);
...
//			this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_validResp, stages[stageMem]);
		...
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_validReq, stages[stageMem-1]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_validReq, stages[stageMem-1]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_validReq, stages[stageMem-2]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_validReq, stages[stageMem-2]);
//		 	this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_validResp, stages[stageMem]);
		...
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_spawn_validResp, stages[spawnStage]);
		this.PutNode("Bits", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_spawn, stages[spawnStage]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_spawn_validReq, stages[spawnStage]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_spawn_validReq, stages[spawnStage]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_spawn_validResp, stages[spawnStage]);
		this.PutNode("UInt", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_spawn_addr, stages[spawnStage]);
		this.PutNode("UInt", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_spawn_addr, stages[spawnStage]);
		this.PutNode("UInt", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_spawn_defaultAddr, stages[spawnStage]);
		this.PutNode("UInt", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_spawn_defaultAddr, stages[spawnStage]);
		this.PutNode("UInt", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_spawn_size, stages[spawnStage]);
		this.PutNode("UInt", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_spawn_size, stages[spawnStage]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.WrMem_spawn_write, stages[spawnStage]);
		this.PutNode("Bool", vexInterfaceStageNames[stageMem] +"", vexInterfaceStageNames[stageMem], BNode.RdMem_spawn_write, stages[spawnStage]);
		
		PipelineStage startSpawnStage = this.vex_core.GetStartSpawnStages().asList().get(0);
		assert(startSpawnStage != null);
		this.PutNode( "Bool", vexInterfaceStageNames[stageMem]+".arbitration.isFiring || io."+language.CreateNodeName(BNode.WrStall, startSpawnStage, ""), vexInterfaceStageNames[startSpawnStage.getStagePos()], BNode.ISAX_spawnAllowed,startSpawnStage);
