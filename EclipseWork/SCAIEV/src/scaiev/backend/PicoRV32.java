...
public class PicoRV32 extends CoreBackend {
	...
	//	toFile.UpdateContent(this.ModFile("picorv32"),"endmodule",  new ToWrite(logicIsISAX,false,true,"",true));
		...
		if(this.op_stage_instr.containsKey(BNode.WrRD))
			for(PipelineStage stage : this.op_stage_instr.get(BNode.WrRD).keySet()) {
				nrIsaxesWithWRRd += this.op_stage_instr.get(BNode.WrRD).get(stage)
						.stream().filter(isax -> !isax.isEmpty()).count();
			}
	...
	private void IntegrateISAX_WrStall() {
		...
		if(this.op_stage_instr.containsKey(BNode.WrStall) | this.op_stage_instr.containsKey(BNode.RdStall) ){
			if(this.ContainsOpInStage(BNode.WrStall, 2)) {
				...
				toFile.ReplaceContent(this.ModFile("picorv32"), "if (!mem_do_prefetch || mem_done) begin", new ToWrite("if("+language.CreateNodeName(BNode.WrStall, stages[2],"")+") begin end else if (!mem_do_prefetch || mem_done) begin",true,false,"cpu_state_stmem: begin")); // Stall if in write state
				toFile.ReplaceContent(this.ModFile("picorv32"), "if (reg_sh == 0) begin", new ToWrite("if("+language.CreateNodeName(BNode.WrStall, stages[2],"")+")begin   end else if (reg_sh == 0) begin",true,false,"cpu_state_shift: begin")); // Stall if in shift state
				toFile.ReplaceContent(this.ModFile("picorv32"), "if ((TWO_CYCLE_ALU || TWO_CYCLE_COMPARE)", new ToWrite("if("+language.CreateNodeName(BNode.WrStall, stages[2],"")+")begin end else if ((TWO_CYCLE_ALU || TWO_CYCLE_COMPARE) && (alu_wait || alu_wait_2)) begin",true,false, "cpu_state_exec: begin")); // stall execute
				toFile.UpdateContent(this.ModFile("picorv32"),  "if (!mem_do_prefetch || mem_done) begin", new ToWrite("if("+language.CreateNodeName(BNode.WrStall, stages[2],"")+")begin end else\n", true, false, "cpu_state_ldmem: begin", true));
				...
				replaceWith = "if (decoder_trigger && !decoder_pseudo_trigger && (!"+language.CreateNodeName(BNode.WrStall, stages[2],"")+" || (~(|cpu_state[3:0])))) begin";
				toFile.ReplaceContent(this.ModFile("picorv32"), replaceText, new ToWrite( replaceWith,false,true, ""));
			...
			if(this.ContainsOpInStage(BNode.WrStall, 1)) {
				...
				toFile.UpdateContent(this.ModFile("picorv32"), "(CATCH_ILLINSN || WITH_PCPI) && instr_trap: begin", new ToWrite("("+language.CreateNodeName(BNode.WrStall, stages[1],"")+") : begin end\n",false,true, "",true));
				...
				toFile.ReplaceContent(this.ModFile("picorv32"),"if (mem_do_prefetch || mem_do_rinst || mem_do_rdata) begin", new ToWrite("if ((mem_do_prefetch || mem_do_rinst || mem_do_rdata) && (!("+language.CreateNodeName(BNode.WrStall, stages[1],"")+") || (cpu_state != cpu_state_ld_rs1))) begin",false,true,""));
				toFile.UpdateContent(this.ModFile("picorv32"),"endcase", new ToWrite("if ("+spawnStall+" "+language.CreateNodeName(BNode.WrStall, stages[1],"")+") cpu_state <= cpu_state_ld_rs1;",true,false,"cpu_state <= cpu_state_ld_rs2;"));				
	...
	private void IntegrateISAX_SpawnRD (){
		...
		if(op_stage_instr.containsKey(BNode.WrRD_spawn) || op_stage_instr.containsKey(BNode.RdMem_spawn) || op_stage_instr.containsKey(BNode.WrMem_spawn)) {
			...
			logic += language.CreateAssign( language.CreateNodeName(BNode.ISAX_spawnAllowed, stages[1], ""),"(cpu_state == cpu_state_ld_rs1) && (mem_state==0)" +stall);
			addLogic(logic);
		}
		
		if(op_stage_instr.containsKey(BNode.WrRD_spawn)) {
			toFile.ReplaceContent(this.ModFile("picorv32"),"if (resetn && cpuregs_write && latched_rd", new ToWrite("if (resetn && cpuregs_write && latched_rd || "+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[spawnStageNum], "")+")", false, true, ""));
			toFile.ReplaceContent(this.ModFile("picorv32"),"if (cpu_state == cpu_state_fetch)",new ToWrite("if ((cpu_state == cpu_state_fetch) || ("+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[spawnStageNum], "")+")) begin",true,false,"cpuregs_wrdata = 'bx;"));
			toFile.UpdateContent(this.ModFile("picorv32"),"case (1'b1)",new ToWrite("("+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[spawnStageNum], "")+"): begin\n"+ tab + "cpuregs_wrdata = "+language.CreateNodeName(BNode.WrRD_spawn, stages[spawnStageNum], "")+";\n"+ tab + "cpuregs_write = 1;\n"+ "end ",true,false,"cpuregs_wrdata = 'bx;"));
			String commit = " begin if("+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[spawnStageNum], "")+")\n"
					+ tab + "cpuregs["+language.CreateNodeName(BNode.WrRD_spawn_addr, stages[spawnStageNum], "")+"] <= cpuregs_wrdata;\n"
					+ "else\n"
	...
	private void IntegrateISAX_SpawnMem() {
		...
		if(op_stage_instr.containsKey(BNode.WrMem_spawn) || op_stage_instr.containsKey(BNode.RdMem_spawn)) {
			...
			String mem_FSM =  "if("+language.CreateNodeName(BNode.WrMem_spawn_validReq, stages[spawnStageNum], "")+") begin\n "
					...
					+ "			mem_wstrb <= mem_la_wstrb & {4{"+language.CreateNodeName(BNode.WrMem_spawn_write, stages[spawnStageNum], "")+"}};\n"
					...
					+ " mem_state_spawn <=0;\n"
					+ "\n";
			addLogic("assign "+language.CreateNodeName(BNode.RdMem_spawn_validResp, stages[spawnStageNum], "")+" = mem_state_spawn && (mem_valid && mem_ready);\n"
					+ rdMem);
	...
	private void IntegrateISAX_Mem() {
		...
		if(rdMem) {
			...
			toFile.ReplaceContent(this.ModFile("picorv32"),"instr_lb    <= is_lb_lh_lw_lbu_lhu", new ToWrite("instr_lb    <= (is_lb_lh_lw_lbu_lhu || ("+earlyValid0+")) && mem_rdata_q[14:12] == 3'b000;", false,true,""));
			toFile.ReplaceContent(this.ModFile("picorv32"),"instr_lh    <= is_lb_lh_lw_lbu_lhu", new ToWrite("instr_lh    <= (is_lb_lh_lw_lbu_lhu || ("+earlyValid0+")) && mem_rdata_q[14:12] == 3'b001;", false,true,""));
			toFile.ReplaceContent(this.ModFile("picorv32"),"instr_lw    <= is_lb_lh_lw_lbu_lhu", new ToWrite("instr_lw    <= (is_lb_lh_lw_lbu_lhu || ("+earlyValid0+")) && mem_rdata_q[14:12] == 3'b010;", false,true,""));
			toFile.ReplaceContent(this.ModFile("picorv32"),"instr_lbu   <= is_lb_lh_lw_lbu_lhu", new ToWrite("instr_lbu    <= (is_lb_lh_lw_lbu_lhu || ("+earlyValid0+")) && mem_rdata_q[14:12] == 3'b100;", false,true,""));
			toFile.ReplaceContent(this.ModFile("picorv32"),"instr_lhu   <= is_lb_lh_lw_lbu_lhu", new ToWrite("instr_lhu    <= (is_lb_lh_lw_lbu_lhu || ("+earlyValid0+")) && mem_rdata_q[14:12] == 3'b101;", false,true,""));
			...
			toFile.ReplaceContent(this.ModFile("picorv32"),"reg_op1 <=",new ToWrite("reg_op1 <= ("+  language.CreateNodeName(BNode.RdMem_addr_valid, stage, "") + ") ? "+ language.CreateNodeName(BNode.RdMem_addr, stage, "")+" :  reg_op1 + decoded_imm;",true,false,"cpu_state_ldmem: begin"));
				...
				toFile.ReplaceContent(this.ModFile("picorv32"),"|{instr_jalr,", new ToWrite("|{instr_jalr, is_lb_lh_lw_lbu_lhu, is_alu_reg_imm, " +language.CreateNodeName(BNode.RdMem_validReq, stages[stageNum-2], "")+"}:",false,true,""));

				toFile.ReplaceContent(this.ModFile("picorv32"),"is_lb_lh_lw_lbu_lhu && !instr_trap", new ToWrite ("(is_lb_lh_lw_lbu_lhu && !instr_trap) ||" + language.CreateNodeName(BNode.RdMem_validReq, stages[stageNum-1], "")+": begin",false,true,""));
		}
	...
	private void addToStallCond(int stage, String newCond) {
		stallConds[stage] += (stallConds[stage].isEmpty() ? "" : " || ") + newCond;
	}
	...
	private void IntegrateISAX_WrPC (){
		...
		if(this.ContainsOpInStage(BNode.WrPC, 1 )) {
			...
			String textToAdd = "if("+language.CreateNodeName(BNode.WrPC_valid, stages[1], "")+" )begin \n"
					...
					+ tab + "end\n"
					+ "end else";
			toFile.ReplaceContent(this.ModFile("picorv32"),"(* parallel_case *)",  new ToWrite (textToAdd, true, false, "cpu_state_ld_rs1: ",true));
		}
		if(this.ContainsOpInStage(BNode.WrPC, 2 )) {
			String textToAdd = "if((cpu_state != cpu_state_ldmem && cpu_state != cpu_state_stmem) | (!mem_do_prefetch && mem_done)) begin\n"
					+ tab + "if("+language.CreateNodeName(BNode.WrPC_valid, stages[2], "")+" )begin // we don't need to check if we are in correct stage. This is done by SCAL \n"
		...
		if(this.ContainsOpInStage(BNode.WrPC, 3)) {		
			String text = "if("+language.CreateNodeName(BNode.WrPC_valid, stages[3], "" )+" )begin \n"
					+ tab + "mem_do_rinst <= 1; \n"
					+ tab + "reg_next_pc <= "+language.CreateNodeName(BNode.WrPC, stages[3], "")+"; \n"
					+ tab + "cpu_state <= cpu_state_fetch; \n"
					+ "end";
			toFile.UpdateContent(this.ModFile("picorv32"),"end",new ToWrite(text,true,false,"cpu_state <= cpu_state_ld_rs1;"));
		}
		...
		if(op_stage_instr.containsKey(BNode.WrPC_spawn))  {
			String spawn  = "if("+language.CreateNodeName(BNode.WrPC_spawn_valid, stages[this.picorv32.maxStage+1], "" )+") begin \n"
					+ tab + "mem_do_rinst <= 1; \n"
					+ tab + "reg_next_pc <= "+language.CreateNodeName(BNode.WrPC_spawn, stages[this.picorv32.maxStage+1], "")+"; \n"
					+ tab + "cpu_state <= cpu_state_fetch; \n"
	...
	private void ConfigPicoRV32 () {
		...
		this.PutNode( " ", "(cpu_state ==  cpu_state_fetch &&  mem_do_rinst) ? mem_rdata_latched :  rdInstr_0_r", "picorv32", BNode.RdInstr,stages[0]);
		...
		this.PutNode( " ", "mem_ready & !mem_instr & (mem_wstrb==0)", "picorv32", BNode.RdMem_spawn_validResp,stages[stageMem]); // in theory should be covered by !stall in ISAX
