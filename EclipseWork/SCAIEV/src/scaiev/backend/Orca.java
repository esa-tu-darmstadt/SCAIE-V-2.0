		...
		if(op_stage_instr.containsKey(BNode.RdFlush)) {
			for(PipelineStage stage : op_stage_instr.get(BNode.RdFlush).keySet()) {
				if(!this.ContainsOpInStage(BNode.WrFlush, stage))  {
					toFile.UpdateContent(this.ModFile(NodeAssignM(BNode.RdFlush, stage)),Parse.declare,	new ToWrite(language.CreateDeclSig(BNode.WrFlush, stage, ""), false, true,""));				
					toFile.UpdateContent(this.ModFile(NodeAssignM(BNode.RdFlush, stage)),Parse.behav,	  new ToWrite(language.CreateLocalNodeName(BNode.WrFlush, stage, "") + " <= '0';\n",false,true,""));
				} else {
					if(stage.getStagePos()==1)
						toFile.UpdateContent(this.ModFile(NodeAssignM(BNode.RdFlush, stage)),Parse.declare,	new ToWrite(language.CreateDeclSig(BNode.WrFlush, stage, ""), false, true,""));				
					
					toFile.UpdateContent(this.ModFile(NodeAssignM(BNode.RdFlush, stage)),Parse.behav,	  new ToWrite(language.CreateLocalNodeName(BNode.WrFlush, stage, "") + " <= "+language.CreateNodeName(BNode.WrFlush, stage, "")+";\n",false,true,""));
				}
			}
		}
	}
	private void IntegrateISAX_WrFlush() {
		...
		for (int stagePos = 3; stagePos >= 1; --stagePos) {
			...
			if (ContainsOpInStage(BNode.WrFlush, stagePos)) {
				toFile.UpdateContent(this.ModFile("orca_core"), Parse.declare, new ToWrite(language.CreateDeclSig(BNode.WrFlush, stage, ""),false,true,""));
			...
			if (stagePos == 1 && !combinedFlushCond.isEmpty()) {
				toFile.ReplaceContent(this.ModFile("orca_core"),"to_decode_valid                    => ", new ToWrite("to_decode_valid                    => to_decode_valid or "+combinedFlushCond+",", false, true , "") );
			}
			if (stagePos == 2 && !combinedFlushCond.isEmpty()) {
				toFile.ReplaceContent(this.ModFile("orca_core"),"quash_decode => ", new ToWrite("quash_decode => to_pc_correction_valid or "+combinedFlushCond+",", false, true , "") );
			}
			if (stagePos == 3 && !combinedFlushCond.isEmpty()) {
				toFile.ReplaceContent(this.ModFile("orca_core")," to_execute_valid            => to_execute_valid,", new ToWrite(" to_execute_valid            => to_execute_valid or "+combinedFlushCond+",", false, true , "") );
			}
		}
	}
...
//				+ "end if;";
		String textIllegal = "if ("+isISAXSignal+" = '1') then\n"
						...
						+ tab+"from_opcode_illegal <= '1';\n"
						+ "end if;";
		toFile.ReplaceContent(this.ModFile("execute"), "from_opcode_illegal <= '1';",new ToWrite(textIllegal,true,false,"when VCP32_OP =>"));
		toFile.ReplaceContent(this.ModFile("execute"), "from_opcode_illegal <= '1';",new ToWrite(textIllegal,true,false,"when VCP64_OP =>"));
		...
		toFile.UpdateContent(this.ModFile("execute"),Parse.declare,  new ToWrite("signal "+language.CreateRegNodeName(is_ISAX, stages[4], "")+" : std_logic;\n",false,true,""));		
		addText = language.CreateTextRegResetStall(language.CreateRegNodeName(is_ISAX, stages[4], ""), isISAXSignal,"(not from_execute_ready)" );
		toFile.UpdateContent(this.ModFile("execute"),Parse.behav,  new ToWrite(addText,false,true,""));
	}
	...
	private void IntegrateISAX_WrStall() {
		...
		if(this.ContainsOpInStage(BNode.WrStall, 0) || this.ContainsOpInStage(BNode.WrFlush, 0)){
			...
			String addtext = " to_ifetch_pause_ifetch <= "+language.CreateNodeName(BNode.WrStall, stages[0], "")+" or ((not (from_decode_incomplete_instruction and ifetch_idle)) and from_execute_pause_ifetch);\n";
		...
		if(this.ContainsOpInStage(BNode.WrStall, 3) || this.ContainsOpInStage(BNode.WrFlush, 3)) {
			...
			toFile.ReplaceContent(this.ModFile(NodeAssignM(BNode.WrStall, stages[3])), "((not vcp_select) or vcp_ready)))",new ToWrite("((not vcp_select) or vcp_ready))));\n",false,true,"", true));
			toFile.ReplaceContent(this.ModFile(NodeAssignM(BNode.WrStall, stages[3])), "from_execute_ready <=",new ToWrite(" from_execute_ready <= "+noStallCond+"((not to_execute_valid) or (from_writeback_ready and",false,true,"", true));
		...
		if(this.ContainsOpInStage(BNode.WrStall, 4) || this.ContainsOpInStage(BNode.WrFlush, 4)) { // mainly for spawn  {
			...
			toFile.ReplaceContent(this.ModFile(NodeAssignM(BNode.WrStall, stages[4])), "from_writeback_ready <= ", new ToWrite("from_writeback_ready <= (not use_after_produce_stall) and (not writeback_stall_from_lsu) and  (not "+language.CreateNodeName(BNode.WrStall, stages[4], "")+");\n", false,true,"", true));
			toFile.ReplaceContent(this.ModFile(NodeAssignM(BNode.WrStall, stages[4])), "to_rf_valid <= to_rf_select_writeable", new ToWrite("to_rf_valid <= "+noStallCond+"to_rf_select_writeable and (from_syscall_valid or", false,true,"", true));
		}
		...
		if(this.ContainsOpInStage(BNode.WrRD, 4) | this.ContainsOpInStage(BNode.WrRD, 3) | this.ContainsOpInStage(BNode.WrRD_spawn, 5) ) {
			...
			if(this.ContainsOpInStage(BNode.WrRD_spawn, 5)) {
				wrrdDataBody +=  "if( "+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[5], "")+" = '1' ) then\n"
						+ tab+ISAX_execute_to_rf_data_s+" <= "+language.CreateNodeName(BNode.WrRD_spawn, stages[5], "")+"; \n"
						+ "    els";
			}
			if(this.ContainsOpInStage(BNode.WrRD, 3)) {
				wrrdDataBody +=  "if( "+language.CreateRegNodeName(BNode.WrRD_valid, stages[3], "")+" = '1' ) then\n"
						+ tab+ISAX_execute_to_rf_data_s+" <= "+language.CreateRegNodeName(BNode.WrRD, stages[3], "")+"; \n"
						+ "    els";
			}
			if(this.ContainsOpInStage(BNode.WrRD, 4)) {
				wrrdDataBody +=  "if( "+language.CreateNodeName(BNode.WrRD_valid, stages[4], "")+" = '1' ) then\n"
						+ tab+ISAX_execute_to_rf_data_s+" <= "+language.CreateNodeName(BNode.WrRD, stages[4], "")+"; \n"
						+ "    els";
			}
			...
			String wrrdValid = ""; 
			if(this.ContainsOpInStage(BNode.WrRD_spawn, 5)) {
				wrrdValid +=  "if( "+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[5], "")+" = '1'  and "+language.CreateNodeName(BNode.WrRD_spawn_addr, stages[5], "")+" /= \"00000\" ) then\n"
						+ tab+ISAX_execute_to_rf_valid_s+" <= '1';\n"
						+ "    els";
			}
			if(this.ContainsOpInStage(BNode.WrRD, 4)) {
				wrrdValid +=  "if( "+language.CreateNodeName(BNode.WrRD_valid, stages[4], "")+" = '1' and (execute_to_rf_select /=\"00000\") ) then\n"
						+ tab+ISAX_execute_to_rf_valid_s+" <=  '1';\n"
						+ "    els";
			}
			...
			// Compute Adddress 
			if(this.ContainsOpInStage(BNode.WrRD_spawn, 5)) {
				String addr = "if( "+language.CreateNodeName(BNode.WrRD_spawn_valid, stages[5], "")+" = '1' ) then\n"
			...
			if(this.ContainsOpInStage(BNode.WrRD, 3)) {
				String newText = language.CreateTextRegResetStall( language.CreateRegNodeName(BNode.WrRD_validData, stages[4], "")  ,   language.CreateNodeName(BNode.WrRD_valid, stages[3], ""), "(not from_execute_ready)");
				toFile.UpdateContent(this.ModFile("execute"),Parse.behav, new ToWrite(newText,false,true,""));
				language.UpdateInterface("orca",BNode.WrRD_validData, "",stages[3],true,false);
				
				newText = language.CreateTextRegResetStall( language.CreateRegNodeName(BNode.WrRD, stages[4], "")  ,   language.CreateNodeName(BNode.WrRD, stages[3], ""), "(not from_execute_ready)");
				toFile.UpdateContent(this.ModFile("execute"),Parse.behav, new ToWrite(newText,false,true,""));
			}
			...
			if(this.ContainsOpInStage(BNode.WrRD, 4)) {
				...
				newText = "from_alu_data <= "+language.CreateNodeName(BNode.WrRD, stages[4], "")+" when "+language.CreateNodeName(BNode.WrRD_validData, stages[4], "")+" = '1' else ISAX_from_alu_data;";
				toFile.UpdateContent(this.ModFile("execute"),Parse.behav, new ToWrite(newText,false,true,""));
				...
				newText = "if ( to_alu_valid = '1' and from_alu_ready = '1') or ("+ language.CreateNodeName(ISAX_FWD_ALU, stages[3], "")+" = '1' and "+language.CreateNodeName(BNode.RdStall, stages[3], "")+" = '0') then";
				toFile.ReplaceContent(this.ModFile("execute"),replace, new ToWrite(newText,false,true,""));				
				...
				newText = 	replace + " and ( "+language.CreateNodeName(BNode.WrRD_validData, stages[4], "")+"  = '0' or  "+language.CreateNodeName(BNode.WrRD_valid, stages[4], "")+" = '1' ) else";
				toFile.ReplaceContent(this.ModFile("execute"),replace, new ToWrite(newText,false,true,""));		
	...
	private void IntegrateISAX_Mem() {
		...
		if( readRequired || writeRequired)  {
			...
			if(readRequired) {
				...
				toFile.UpdateContent(this.ModFile("load_store_unit"),Parse.behav,new ToWrite(language.CreateText1or0("read_s", language.CreateNodeName(BNode.RdMem_validReq,stage,"")+" = '1' or ((opcode(5) = LOAD_OP(5)) and ("+language.CreateNodeName(is_ISAX, stages[3], "")+" = '0'))"), false,true,""));
		...
		if(this.op_stage_instr.containsKey(BNode.WrMem_spawn) ||this.op_stage_instr.containsKey(BNode.RdMem_spawn) ) {
			...
			String rdAddrLogic = language.CreateLocalNodeName(BNode.RdMem_spawn_defaultAddr, stages[this.orca_core.maxStage+1], "") + " <= oimm_address;\n";
			...
			this.op_stage_instr.getOrDefault(BNode.WrMem_spawn, new HashMap<>()).getOrDefault(spawnStage, new HashSet<>())
				.stream().filter(isax -> !isax.isEmpty())
				.forEach(isax -> allMemSpawn.add(isax));
			this.op_stage_instr.getOrDefault(BNode.RdMem_spawn, new HashMap<>()).getOrDefault(spawnStage, new HashSet<>())
				.stream().filter(isax -> !isax.isEmpty())
				.forEach(isax -> allMemSpawn.add(isax));
	...
	private void IntegrateISAX_WrPC() {
		...
		if(!array_PC_clause.isEmpty() || op_stage_instr.containsKey(BNode.WrPC_spawn)) {
			...
			for(int stagePos =0; stagePos <=orca_core.GetNodes().get(BNode.WrPC).GetLatest().asInt()+1; stagePos++) {
				...
				if(array_PC_clause.containsKey(stagePos) || (stagePos == (orca_core.GetNodes().get(BNode.WrPC).GetLatest().asInt()+1))) {
					if(this.ContainsOpInStage(BNode.WrPC, max_stage+1) && stagePos==0) { // not supported anymore!!!
						...
						PC_text += "if("+language.CreateNodeName(BNode.WrPC_spawn_valid, stages[max_stage+1],"")+" = '1') then\n"+tab.repeat(2)+"ISAX_to_pc_correction_data_s <= "+language.CreateNodeName(BNode.WrPC_spawn, stages[max_stage+1],"")+";\n";
			...
			String cond_noWrPC_todecode = IntStream.range(2, max_stage).mapToObj(i -> array_PC_clause.get(i)).filter(validExpr -> validExpr != null).reduce((a,b)->a+" || "+b).orElse("'0'");
			String cond_noWrPC_toexecute = IntStream.range(4, max_stage).mapToObj(i -> array_PC_clause.get(i)).filter(validExpr -> validExpr != null).reduce((a,b)->a+" || "+b).orElse("'0'");
			toFile.ReplaceContent(sub_top_file, "to_decode_valid <=", new ToWrite("to_decode_valid <= from_ifetch_valid and (not to_pc_correction_valid or "+cond_noWrPC_todecode+");",false, true, ""));
			toFile.ReplaceContent(sub_top_file, "to_execute_valid <=", new ToWrite("to_execute_valid <= from_decode_valid and (not to_pc_correction_valid or "+cond_noWrPC_toexecute+");",false, true, ""));
			//if(this.ContainsOpInStage(BNode.WrFlush, 2))
			//	toFile.ReplaceContent(sub_top_file, "quash_decode =>", new ToWrite("quash_decode        => to_pc_correction_valid or "+language.CreateNodeName(BNode.WrFlush, stages[2],"")+",",true, false, "D : decode"));
	...
	private boolean ContainsOpInStage(SCAIEVNode operation, int stage) {
		return ContainsOpInStage(operation, stages[stage]);
	}
	...
	private void ConfigOrca() {
		...
		int stageMem = this.orca_core.GetNodes().get(BNode.RdMem).GetLatest().asInt();
	 	...
	 	this.PutNode( "std_logic", "not (from_stage1_valid) or not ("+language.CreateLocalNodeName(ISAX_frwrd_to_stage1_ready, stages[2], "")+")", "decode", BNode.RdStall, stages[2]);
	 	String rdStall3 = 
			  "(not to_execute_valid) or \n"
	 	...
	 	this.PutNode( "std_logic", "not (from_syscall_valid or from_lsu_valid or from_branch_valid or from_alu_valid or "+language.CreateRegNodeName(is_ISAX, stages[4], "")+") or not (from_writeback_ready)", "execute", BNode.RdStall, stages[4]);
...
//		 	op_stage_instr.computeIfAbsent(BNode.RdFlush, node_ -> new HashMap<>()).computeIfAbsent(stages[2], stage_ -> new HashSet<>()).add("");
//	 		toExecuteValidNoflushCond += " and not " + language.CreateNodeName(BNode.RdFlush,stages[2], "");
	 	...
	 	this.PutNode( "std_logic", "(from_syscall_valid or from_lsu_valid or from_branch_valid or from_alu_valid or "+language.CreateRegNodeName(is_ISAX, stages[4], "")+")", "execute", BNode.RdInStageValid, stages[4]);
	 	...
	 	this.PutNode( "std_logic"       ,"" ,"orca_core", BNode.WrMem_spawn_validResp,stages[spawnStage]); // ? ((lsu_oimm_waitrequest = '0')  or (lsu_oimm_readdatavalid = '1'))
