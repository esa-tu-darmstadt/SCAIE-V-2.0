...
public class Bluespec extends GenerateText {
	 ...
	 */	 
	@Override public String CreateBasicNodeName(SCAIEVNode operation, PipelineStage stage, String instr, boolean familyname) {
	...
	public void  UpdateInterface(String top_module,SCAIEVNode operation, String instr, PipelineStage stage, boolean top_interface, boolean assigReg) {
		...
		while(!prev_module.contentEquals(top_module)) {
			...
			if(!current_module.contentEquals(top_module) || top_interface) {  // top file should just instantiate signal in module instantiation and not generate top interface if top_interface = false
				...
				this.toFile.UpdateContent(coreBackend.ModInterfFile(current_module), "endinterface",new ToWrite(additional_text+CreateMethodDecl(operation,stage,instr,false)+";\n",true,false,"interface",true));		
			...
			if(prev_module.contentEquals("")) { // no previous files => this is bottom file
				...
				String forwardSig = CreateMethodDecl(operation,stage,instr,true)+" = "+instName + ".met_"+CreateNodeName(operation, stage,instr);
				if(coreBackend.NodeIsInput(operation, stage))
					forwardSig += "(x);\n";
				else 
					forwardSig += ";\n";
