...
class PipelineTest {
	...
	void testFront(long seed) {
		...
		for (int i = 0; i < builtPipeline.coreDepthBuckets.size(); ++i) {
			...
			builtPipeline.coreDepthBuckets.get(i).stream()
		...
		for (int iProbe = 0; iProbe < numSurfaceProbes; ++iProbe)
		{
			...
			Assertions.assertTrue(frontBucket.stream().allMatch(a -> frontBucketAllStages.contains(a)), "Unexpected streamNext_bfs result contents");
			if (iFrontBucket+1 < builtPipeline.coreDepthBuckets.size()) {
				//Check that we haven't collected 
				Assertions.assertTrue(builtPipeline.coreDepthBuckets.get(iFrontBucket+1)
						.stream()
						.allMatch(a -> !frontBucketAllStages.contains(a)), 
			...
			Assertions.assertTrue(
				otherBucketAllStages.stream().allMatch(otherStage -> 
					//
			...
			Assertions.assertTrue(
				otherBucketAllStages.stream().allMatch(otherStage -> 
					...
					(front.isBefore(otherStage, false) == (
							...
							|| ((iFrontBucket == iOtherBucket) && !otherBucket.contains(otherStage))
						))
				));
			...
			Assertions.assertTrue(
				otherBucketAllStages.stream().allMatch(otherStage -> 
					front.isAroundOrAfter(otherStage, false) == front.isAround(otherStage) || front.isAfter(otherStage, false) 
				));
			Assertions.assertTrue(
				otherBucketAllStages.stream().allMatch(otherStage -> 
					front.isAroundOrBefore(otherStage, false) == front.isAround(otherStage) || front.isBefore(otherStage, false) 
				));
		}
		...
		for (PipelineStage subPipelinedStage : builtPipeline.subPipelinedStages) {
			...
			Assertions.assertTrue(childList.stream().allMatch(childStage -> 
				rootFront.isAround(childStage)
				&& subPipelinedStageFront.isAround(childStage)
				&& bothFront.isAround(childStage)
				&& bothFront.isAroundOrBefore(childStage, false)
				&& bothFront.isAroundOrAfter(childStage, false)));
