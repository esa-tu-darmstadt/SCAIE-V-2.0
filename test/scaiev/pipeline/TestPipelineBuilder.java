...
public class TestPipelineBuilder {
  ...
  public GeneratedPipeline build(int minDepthCore, int maxDepthCore) {
      ...
      for (int iEntry = 0; iEntry < nEntries; ++iEntry) {
        ...
        if (iBucket == iSubpipedBucket && iEntry == iSubpipeEntry) {
          ...
          for (int iSubpipeline = 0; iSubpipeline < nSubpipelines; ++iSubpipeline) {
            ...
            assert (curStage.getChildren()
                        .get(iSubpipeline)
                        .streamNext_bfs()
                        .allMatch(childStage
                                  -> (childStage.prev.size() == 1 ||
                                      childStage.getStagePos() == 0) // Has one prev, one next (except first, last respectively)
          ...
          if (iEntry + 1 < nEntries)
            iSubpipeEntry =
                rand.ints(iEntry + 1, nEntries + 1).findFirst().getAsInt(); // Explicit off-by-one to add a chance not to select another.
        }
      }
    }
    ...
    for (int iBucket = 1; iBucket < nDepthBuckets; ++iBucket) {
      ...
      List<PipelineStage> prevBucketEnds = ret.coreDepthBuckets.get(iBucket - 1)
                                               .stream()
      ...
      bucketEntries.forEach(bucketEntryStage -> {
        ...
        while (
            iConnectTo < prevBucketEnds.size() - 1 &&
            (iConnectTo = rand.ints(iConnectTo + 1, prevBucketEnds.size() + (iConnectTo >= 0 ? 1 : 0) /*Explicit conditional off-by-one*/)
                              .findFirst()
                              .getAsInt()) < prevBucketEnds.size()) {
    ...
    {
      int maxStage =
          ret.rootStage.getAllChildrenRecursive().flatMap(stage -> stage.getGlobalStageID().stream()).max(Integer::compare).get();
      ...
      assert (ret.rootStage.getAllChildrenRecursive().flatMap(stage -> stage.getGlobalStageID().stream()).distinct().count() ==
              maxStage + 1);
    }
    ...
    assert (ret.rootStage.getAllChildrenRecursive().allMatch(
        stage
        -> stage.getStagePos() >= 0 &&
    ...
    assert (ret.rootStage.getAllChildrenRecursive().allMatch(
        child -> child.getNext().stream().allMatch(next -> next.getPrev().contains(child))));
    assert (ret.rootStage.getAllChildrenRecursive().allMatch(
        child -> child.getPrev().stream().allMatch(prev -> prev.getNext().contains(child))));
